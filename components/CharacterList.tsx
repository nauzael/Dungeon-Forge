...
const DeleteConfirmModal: React.FC<{ char: Character; onConfirm: () => void; onCancel: () => void }> = ({ char, onConfirm, onCancel }) => {
  return createPortal(
    <div 
    ...
    >
      ...
      >
        <div className="bg-gradient-to-br from-red-500/20 to-red-900/10 p-6 pb-4">
          ...
          <h3 className="text-xl font-black text-white text-center mb-2">Delete Character</h3>
          <p className="text-sm text-slate-400 text-center">
            Are you sure you want to delete <span className="text-white font-bold">{char.name}</span>? This action cannot be undone.
          </p>
        </div>
  ...
  return (
    <>
      {showDeleteModal && (
        <DeleteConfirmModal 
          char={char} 
          onConfirm={() => { setShowDeleteModal(false); onDelete(char.id); }}
