...
const CampaignResources: React.FC<{ partyId: string }> = ({ partyId }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) {
      console.log('[Atlas] No file selected');
      return;
    }

    console.log('[Atlas] File selected:', file.name, file.size, file.type);
    setIsUploading(true);

    try {
      const result = await uploadResourceImage(file);

      console.log('[Atlas] uploadResourceImage result:', result);
      if (result) {
        setNewRes((prev) => ({
          ...prev,
          url: result.fullUrl,
  ...
  const handleAdd = async () => {
    console.log('[Atlas] handleAdd called, newRes:', newRes);
    ...
    setIsLoading(true);
    console.log('[Atlas] Calling addPartyResource...');
    ...
    try {
      ...
      console.log('[Atlas] Resource data to insert:', JSON.stringify(resourceData, null, 2));
      const added = await addPartyResource(resourceData);

      console.log('[Atlas] addPartyResource result:', added);
      if (added) {
        // Update thumbnail in background if we have one
        if (newRes.thumbnail_url) {
          updateResourceThumbnail(added.id, newRes.thumbnail_url).then((ok) => {
            console.log('[Atlas] Thumbnail update:', ok ? 'OK' : 'FAILED');
            fetchResources();
          });
        }
      ...
      console.error('[Atlas] handleAdd error:', err);
    ...
    >
        ...
        <div className="bg-slate-800/80 backdrop-blur-md rounded-2xl p-5 border border-blue-500/20 space-y-4 animate-slideDown shadow-2xl">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between bg-white/5 p-3 rounded-xl border border-white/5">
              ...
              >
                <div
                  className={`absolute top-1 size-3 rounded-full bg-white transition-all ${newRes.is_persistent ? 'right-1' : 'left-1'}`}
                ></div>
              </button>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                className="flex-1 py-3 bg-white/5 text-slate-400 font-bold uppercase text-[10px] tracking-widest rounded-xl"
              >
                Cancel
              </button>
              <button
                ...
                onClick={handleAdd}
                className={`flex-1 py-3 text-white font-bold uppercase text-[10px] tracking-widest rounded-xl shadow-lg transition-all ${isLoading || isUploading ? 'bg-slate-700' : 'bg-blue-600'}`}
              >
      ...
      <div className="grid grid-cols-1 gap-6 pb-20">
        ...
        {resources.map((res) => (
          <div
            key={res.id}
            className="group bg-[#1e293b] rounded-3xl overflow-hidden border border-white/5 shadow-2xl transition-all hover:border-blue-500/20"
          >
            <div
            ...
            >
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-2">
                <span className="bg-black/60 backdrop-blur-md text-[8px] font-black uppercase text-blue-400 px-2 py-1 rounded-full border border-blue-500/20 tracking-widest w-fit">
                  {res.type}
                </span>
              ...
              <div className="absolute bottom-4 left-4 right-4">
                <h3 className="text-lg font-black italic uppercase tracking-tight text-white leading-tight drop-shadow-lg">
                  {res.title}
                </h3>
              </div>
            </div>
        ...
        >
          <div className="flex items-center justify-between mb-3 gap-2 shrink-0">
            <div className="flex items-center gap-2 min-w-0">
              <span className="px-2 py-0.5 rounded-full bg-blue-600/90 text-[8px] font-black uppercase text-white tracking-widest">
          ...
          <div
            ...
            className="flex-1 flex items-center justify-center overflow-hidden rounded-2xl touch-none cursor-grab active:cursor-grabbing"
          ...
          >
            <div ref={previewZoom.contentRef} style={previewZoom.transformStyle}>
              <img
                ...
                className="max-w-[92vw] max-h-[calc(100vh-9rem)] object-contain rounded-lg select-none"
          ...
          <div className="flex justify-center items-center gap-2 mt-4">
            <button
              ...
              className="size-12 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center text-white backdrop-blur-sm active:bg-white/40"
            >
            ...
            <button
              ...
              className="size-12 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center text-white backdrop-blur-sm active:bg-white/40 ml-2"
            >
