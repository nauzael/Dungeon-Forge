...
const SheetTabs: React.FC<SheetTabsProps> = ({
  ...
  const initiateLevelUp = () => {
    ...
    try {
      ...
      console.error('Failed to create level snapshot:', error);
    }
    ...
    >
      ...
      <div className="sticky top-0 z-30 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md border-b border-black/5 dark:border-white/5 px-4 py-4 flex items-center gap-4 pt-[calc(1.25rem+env(safe-area-inset-top))]">
        ...
        <div className="flex flex-1 items-center gap-4 min-w-0">
          ...
          >
            <div
              className={`size-14 rounded-2xl overflow-hidden shadow-xl ring-2 ring-primary/20 bg-slate-100 dark:bg-black/40 transition-all ${!isReadOnly ? 'group-active:scale-95 cursor-pointer' : ''}`}
            >
          ...
          <div className="flex flex-col min-w-0 flex-1">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-400 dark:text-slate-500 truncate flex items-center gap-1">
              ...
              {character.level >= 20 && (
                <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white text-[9px] font-bold rounded-full shadow-sm">
                  MAX
                </span>
              )}
              <span>
        ...
        <LevelResetModal
          ...
          onRestore={(snapshotId) => {
            try {
              ...
              alert('Failed to restore snapshot. Please check the console for details.');
            }
          }}
        ...
        >
          ...
          >
            <div className="flex justify-end mb-3 shrink-0">
              <button
                onClick={() => { setZoomImage(null); zoom.resetZoom(); }}
                className="size-10 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center text-white backdrop-blur-sm active:bg-white/40"
              >
            ...
            >
              <div ref={zoom.contentRef} style={zoom.transformStyle}>
                <img
                  src={zoomImage}
                  className="max-w-[90vw] max-h-[calc(100vh-9rem)] object-contain rounded-lg select-none"
                  alt="Zoomed"
                  draggable={false}
                />
              </div>
