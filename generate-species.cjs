...
const OUTPUT = 'Data/species/species-en.ts';

let out = `import { DetailData } from '../../types';
...
const speciesData = {
  ...
  dragonborn: {
    ...
    subspecies: [
      { name: 'Black Dragon', description: 'Acid Resistance and 15ft Line Breath Weapon.', traits: [] },
      { name: 'Blue Dragon', description: 'Lightning Resistance and 15ft Line Breath Weapon.', traits: [] },
      { name: 'Brass Dragon', description: 'Fire Resistance and 15ft Line Breath Weapon.', traits: [] },
      { name: 'Bronze Dragon', description: 'Lightning Resistance and 15ft Line Breath Weapon.', traits: [] },
      { name: 'Copper Dragon', description: 'Acid Resistance and 15ft Line Breath Weapon.', traits: [] },
      { name: 'Gold Dragon', description: 'Fire Resistance and 15ft Cone Breath Weapon.', traits: [] },
      { name: 'Green Dragon', description: 'Poison Resistance and 15ft Cone Breath Weapon.', traits: [] },
      { name: 'Red Dragon', description: 'Fire Resistance and 15ft Cone Breath Weapon.', traits: [] },
      { name: 'Silver Dragon', description: 'Cold Resistance and 15ft Cone Breath Weapon.', traits: [] },
      { name: 'White Dragon', description: 'Cold Resistance and 15ft Cone Breath Weapon.', traits: [] }
    ]
  },
  dwarf: {
    name: 'Dwarf',
...
Object.keys(speciesData).forEach(key => {
  ...
  const exportName = key + 'En';
  
  out += `\nexport const ${exportName}: DetailData = {\n`;
  out += `  name: '${sp.name}',\n`;
  out += `  description: '${sp.description.replace(/'/g, "\\'")}',\n`;
  out += `  size: '${sp.size}',\n`;
  out += `  speed: ${sp.speed},\n`;
  out += `  traits: [\n`;
  sp.traits.forEach(t => {
    out += `    { name: '${t.name.replace(/'/g, "\\'")}', description: '${t.description.replace(/'/g, "\\'").replace(/\n/g, '\\n')}' },\n`;
  });
  out += `  ],\n`;
  out += `  subspecies: [\n`;
  sp.subspecies.forEach(sub => {
    out += `    {\n`;
    out += `      name: '${sub.name}',\n`;
    out += `      description: '${sub.description.replace(/'/g, "\\'")}',\n`;
    out += `      traits: [\n`;
...
console.log(`Written to ${OUTPUT} with ${Object.keys(speciesData).length} species`);
