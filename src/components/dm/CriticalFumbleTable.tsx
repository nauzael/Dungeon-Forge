...
const CriticalFumbleTable: React.FC<CriticalFumbleTableProps> = ({ onApplyDamage }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Table Toggle */}
      <div className="flex bg-white/5 p-1 rounded-radius-lg gap-1">
        <button
          onClick={() => { setActiveTable('critical'); setCurrentResult(null); setDiceRoll(null); }}
          className={`flex-1 py-3 rounded-radius-md font-bold text-sm flex items-center justify-center gap-2 transition-all ${
            activeTable === 'critical' 
              ? 'bg-amber-500 text-white shadow-elev-modal' 
        ...
        <button
          ...
          className={`flex-1 py-3 rounded-radius-md font-bold text-sm flex items-center justify-center gap-2 transition-all ${
            activeTable === 'fumble' 
              ? 'bg-red-600 text-white shadow-elev-modal' 
              : 'text-slate-400 hover:text-white hover:bg-white/5'
          }`}
        >
          ...
          <div className="grid grid-cols-5 sm:grid-cols-10 gap-2">
            {Array.from({ length: 20 }, (_, i) => i + 1).map(num => (
              <button
                ...
                className={`aspect-square rounded-radius-md flex items-center justify-center font-black text-sm transition-all border ${
                  diceRoll === num
                    ? activeTable === 'critical'
                      ? 'bg-amber-500 border-amber-400 text-white scale-110 shadow-elev-modal'
                      : 'bg-red-600 border-red-500 text-white scale-110 shadow-elev-modal'
      ...
      {/* Rolling Animation */}
      {isRolling && (
        <div className="bg-black/40 rounded-radius-lg p-8 text-center">
      ...
      {currentResult && !isRolling && (
        <div className={`rounded-radius-lg overflow-hidden ${
          ...
          <div className="p-4 space-y-4">
            <div className="bg-white/5 border border-white/10 rounded-radius-lg p-3">
              <div className="flex items-center gap-2 mb-2">
                <span className="material-symbols-outlined text-amber-500 text-lg">swords</span>
                <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">Physical Attack</span>
              </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-radius-lg p-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">Magic Attack</span>
              </div>
              <p className="text-white text-sm leading-relaxed">
                {currentResult.magic}
              </p>
            ...
            <div className="flex gap-2 pt-2">
              <button
                onClick={handleRandom}
                className="flex-1 py-3 bg-white/10 hover:bg-white/20 text-white rounded-radius-lg text-sm font-bold flex items-center justify-center gap-2 transition-colors"
              >
                <span className="material-symbols-outlined text-sm">casino</span>
                Tirar de nuevo
              </button>
              ...
              {onApplyDamage && activeTable === 'critical' && currentResult.roll === 1 && (
                <button
                  ...
                  className="flex-1 py-3 bg-amber-500 hover:bg-amber-400 text-white rounded-radius-lg text-sm font-bold flex items-center justify-center gap-2 transition-colors"
                >
              ...
              <button
                onClick={reset}
                className="px-4 py-3 bg-white/5 hover:bg-white/10 text-slate-400 rounded-radius-lg text-sm transition-colors"
              >
                Cerrar
              </button>
            </div>
      ...
      {/* Info Text */}
      {!activeTable && !isRolling && (
        <div className="bg-white/5 rounded-radius-lg p-4">
          <p className="text-slate-400 text-xs text-center leading-relaxed">
            Select a table to view Critical or Fumble effects. 
            You can choose the result manually or roll the virtual die.
          </p>
        </div>
      )}
