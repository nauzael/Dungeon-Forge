...
const MonsterBuilder: React.FC<MonsterBuilderProps> = ({ playerLevels }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-radius-2xl p-6 shadow-2xl space-y-5"
        style={{
        ...
        <div className="grid grid-cols-4 gap-2">
          {Object.entries(budgets).map(([diff, xp]) => (
            <div
              key={diff}
              className={`flex flex-col items-center rounded-radius-lg p-2 ${difficulty.toLowerCase() === diff.toLowerCase() ? 'border-blue-500/50 bg-blue-500/10' : ''}`}
              style={{
        ...
        <div className="pt-2">
          ...
          <div className="h-3 w-full rounded-radius-pill overflow-hidden shadow-inner"
            style={{
              backgroundColor: 'var(--color-background-secondary)',
              borderColor: 'var(--color-border)',
              borderWidth: '1px',
            ...
            <div
              className={`h-full transition-all duration-motion-hp ${difficulty === 'DEADLY' ? 'bg-red-500' : difficulty === 'HARD' ? 'bg-orange-500' : 'bg-emerald-500'}`}
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-center px-2">
          ...
          <button
            ...
            className="size-8 rounded-radius-pill bg-blue-600 flex items-center justify-center border-4 border-[#0f172a] shadow-elev-modal shadow-blue-500/20 active:scale-95 transition-transform"
          >
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
          {POPULAR_MONSTERS.slice(0, 12).map((monster) => (
            <button
              ...
              className="flex-shrink-0 bg-[#1e293b] rounded-radius-lg px-3 py-2 border border-white/5 hover:border-blue-500/30 transition-all active:scale-95 group"
            >
        ...
        <div className="bg-slate-800/80 backdrop-blur-md rounded-radius-xl p-5 border border-blue-500/20 space-y-4 animate-slideDown shadow-2xl">
          ...
          <div className="relative">
            ...
            {suggestions.length > 0 && (
              <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-slate-900 border border-white/10 rounded-radius-lg overflow-hidden shadow-2xl max-h-48 overflow-y-auto">
          ...
          {/* Monster Preview */}
          {name && (
            <div className="flex items-center gap-3 p-3 bg-black/20 rounded-radius-lg border border-white/5">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1">
              ...
              <select
                ...
                className="w-full bg-white/5 border border-white/10 rounded-radius-lg px-2 py-3 text-xs text-white appearance-none outline-none focus:border-blue-500/50 truncate"
              >
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-radius-lg px-2 py-3 text-xs text-white outline-none focus:border-blue-500/50"
              />
            </div>
            <div className="space-y-1">
          ...
          <button
            ...
            className="w-full py-2 bg-emerald-600/20 border border-emerald-500/30 text-emerald-400 font-bold uppercase text-[9px] tracking-widest rounded-radius-lg hover:bg-emerald-600/30 transition-colors flex items-center justify-center gap-2"
          >
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              className="w-full bg-white/5 border border-white/10 rounded-radius-lg px-4 py-3 text-sm text-white appearance-none outline-none focus:border-blue-500/50"
            >
          ...
          <div className="flex gap-3 pt-2">
            <button
              onClick={resetForm}
              className="flex-1 py-3 bg-white/5 text-slate-400 font-bold uppercase text-[10px] tracking-widest rounded-radius-lg hover:bg-white/10 transition-colors"
            >
              Cancelar
            </button>
            <button
              onClick={handleAdd}
              className="flex-1 py-3 bg-blue-600 text-white font-bold uppercase text-[10px] tracking-widest rounded-radius-lg shadow-elev-modal shadow-blue-900/40 hover:bg-blue-500 transition-colors"
            >
              Invocar
            </button>
          </div>
          ...
          {monsters.length > 0 && (
            <div className="flex gap-2">
              <select
                ...
                className="bg-white/5 border border-white/10 rounded-radius-md px-2 py-1 text-[10px] text-slate-400 outline-none"
              >
              ...
              <button
                onClick={() => setMonsters([])}
                className="px-2 py-1 bg-red-500/20 border border-red-500/30 text-red-400 text-[10px] font-bold rounded-radius-md hover:bg-red-500/30"
              >
                Limpiar
              </button>
            </div>
        ...
        {filteredMonsters.length === 0 ? (
          ...
          <div className="grid gap-3 pb-20">
            {filteredMonsters.map((m) => (
              <div
                key={m.id}
                className={`bg-[#1e293b] rounded-radius-xl border border-white/5 shadow-xl overflow-hidden transition-all ${isDead(m) ? 'opacity-50 grayscale' : ''}`}
              >
                ...
                >
                  <div className="flex gap-4">
                    <div
                      className={`size-12 rounded-radius-lg border flex items-center justify-center ${isDead(m) ? 'bg-slate-700 border-slate-600 text-slate-500' : 'bg-red-500/10 border-red-500/20 text-red-500'}`}
                    >
                  ...
                  <div className="flex-1 max-w-[120px] mx-4">
                    ...
                    <div className="h-2 w-full bg-slate-800 rounded-radius-pill overflow-hidden border border-white/5">
                      <div
                        className={`h-full transition-all duration-motion-slow ${getHpColor(getHpPercent(m.currentHp, m.maxHp))}`}
                ...
                {expandedMonster === m.id && (
                  <div className="border-t border-white/5 p-4 space-y-4 bg-[#0f172a]/50 animate-slideDown">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <div className="flex-1 flex gap-2">
                          <input
                            ...
                            className="flex-1 bg-red-500/10 border border-red-500/30 rounded-radius-lg px-3 py-2 text-sm text-white placeholder-red-400/50 outline-none focus:border-red-500/50"
                          />
                          <button
                            onClick={() => {
                              if (damageInput[m.id])
                            ...
                            className="px-4 py-2 bg-red-600 text-white font-bold text-xs rounded-radius-lg"
                          >
                            Damage
                          </button>
                        </div>
                        <div className="flex-1 flex gap-2">
                          <input
                            ...
                            className="flex-1 bg-emerald-500/10 border border-emerald-500/30 rounded-radius-lg px-3 py-2 text-sm text-white placeholder-emerald-400/50 outline-none focus:border-emerald-500/50"
                          />
                          <button
                            onClick={() => {
                              if (healInput[m.id])
                            ...
                            className="px-4 py-2 bg-emerald-600 text-white font-bold text-xs rounded-radius-lg"
                          >
                            Curar
                          </button>
                        </div>
                      ...
                      {/* HP Bar - Full */}
                      <div className="h-4 w-full bg-slate-800 rounded-radius-pill overflow-hidden border border-white/5">
                        <div
                          className={`h-full transition-all duration-motion-slow ${getHpColor(getHpPercent(m.currentHp, m.maxHp))}`}
                          style={{ width: `${getHpPercent(m.currentHp, m.maxHp)}%` }}
                        ></div>
                        ...
                        {Object.entries(CONDITIONS).map(([key, cond]) => {
                          ...
                          return (
                            <button
                              ...
                              className={`px-2 py-1 rounded-radius-md text-[9px] font-bold uppercase flex items-center gap-1 transition-all ${isActive ? `${cond.color} bg-white/10 border border-current` : 'text-slate-600 bg-white/5 border border-white/5 hover:border-white/20'}`}
                            >
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="space-y-1.5">
                        {m.abilities.map((ability) => (
                          <div
                            key={ability.id}
                            className="flex items-center justify-between bg-white/5 rounded-radius-lg px-3 py-2 border border-white/5"
                          >
                            ...
                            {ability.maxUses > 0 ? (
                              <div className="flex items-center gap-1">
                                <button
                                  ...
                                  className={`size-7 rounded-radius-md flex items-center justify-center text-xs font-bold ${ability.uses >= ability.maxUses ? 'bg-slate-700 text-slate-500' : 'bg-blue-600 text-white hover:bg-blue-500'}`}
                                >
                    ...
                    <div className="flex gap-2 pt-2">
                      <button
                        ...
                        className="flex-1 py-2 bg-yellow-500/20 border border-yellow-500/30 text-yellow-400 font-bold uppercase text-[9px] tracking-widest rounded-radius-lg hover:bg-yellow-500/30 transition-colors"
                      >
                        Short Rest (+{Math.floor(m.maxHp / 2)} HP)
                      </button>
                      <button
                        onClick={() => resetLongRest(m.id)}
                        className="flex-1 py-2 bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 font-bold uppercase text-[9px] tracking-widest rounded-radius-lg hover:bg-emerald-500/30 transition-colors"
                      >
                        Long Rest (Full HP)
                      </button>
                    </div>
                    ...
                    <div className="flex gap-2 pt-2">
                      <button
                        ...
                        className="flex-1 py-2 bg-white/5 border border-white/10 text-slate-400 font-bold uppercase text-[9px] tracking-widest rounded-radius-lg hover:bg-white/10 transition-colors"
                      >
                        Duplicar
                      </button>
                      <button
                        onClick={() => setMonsters((prev) => prev.filter((x) => x.id !== m.id))}
                        className="flex-1 py-2 bg-red-500/20 border border-red-500/30 text-red-400 font-bold uppercase text-[9px] tracking-widest rounded-radius-lg hover:bg-red-500/30 transition-colors"
                      >
                        Eliminar
                      </button>
                    </div>
