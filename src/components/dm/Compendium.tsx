...
const Compendium: React.FC = () => {
    ...
    return (
        <div className="space-y-6 lg:max-w-4xl lg:mx-auto">
            {/* Search and Filter */}
            <div className="bg-slate-800/80 backdrop-blur-xl rounded-radius-xl p-4 border border-white/5 space-y-4 shadow-xl sticky top-2 z-30">
                <div className="flex items-center gap-3 bg-black/20 rounded-radius-lg px-4 py-3 border border-white/5 focus-within:border-blue-500/50 transition-all">
                ...
                <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none snap-x">
                    {CATEGORIES.map(cat => (
                        <button 
                            ...
                            className={`px-4 py-2 rounded-radius-md text-[10px] font-bold uppercase tracking-wider transition-all border snap-start shrink-0 ${filter === cat ? 'bg-blue-600 border-blue-500 text-white shadow-elev-modal shadow-blue-900/40' : 'bg-white/5 border-white/10 text-slate-400'}`}
                        >
                            {cat === 'All' ? 'All' : cat}
                        </button>
                    ))}
                ...
                {filtered.length === 0 ? (
                    ...
                    filtered.map(item => (
                        <div 
                            ...
                            className="group bg-slate-800/40 backdrop-blur-md rounded-radius-xl p-5 border border-white/5 shadow-elev-modal hover:border-blue-500/30 transition-all cursor-pointer active:scale-[0.98]"
                        >
                            <div className="flex justify-between items-start mb-3">
                                ...
                                <span className="text-[8px] font-bold bg-blue-500/10 text-blue-400 px-2 py-0.5 rounded-radius-sm border border-blue-500/20 uppercase tracking-widest">{item.category}</span>
                            </div>
            ...
            {selectedItem && (
                ...
                >
                    <div 
                        className="bg-slate-900 rounded-radius-2xl p-6 sm:p-8 w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-white/10 shadow-2xl animate-slideUp relative"
                    ...
                    >
                        <div className="flex justify-between items-start mb-6 sticky top-0 bg-slate-900/95 backdrop-blur-md py-2 z-10 border-b border-white/5">
                            <div>
                                <span className="text-[9px] font-bold bg-blue-600/20 text-blue-400 px-2 py-1 rounded-radius-sm border border-blue-400/20 uppercase tracking-widest mb-2 inline-block">
                            ...
                            <button 
                                onClick={handleCloseModal}
                                className="size-10 rounded-radius-lg bg-white/5 flex items-center justify-center border border-white/10 hover:bg-white/10 transition-all text-slate-400"
                            >
                            ...
                            {selectedItem.fullInfo ? (
                                <div className="text-slate-300">
                                    {selectedItem.fullInfo.split('\n\n').map((block, idx) => {
                                            ...
                                            return (
                                                <ul key={idx} className="space-y-2 my-4">
                                                    {trimmedBlock.split('\n').map((li, i) => {
                                                        ...
                                                        return (
                                                            <li key={i} className="flex gap-3 text-sm leading-relaxed bg-white/3 p-3 rounded-radius-lg border border-white/5 font-normal">
                                                                <span className="text-blue-500 font-bold">•</span>
                                                                <span dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(liContent.replace(/\*\*(.*?)\*\*/g, '<b class="text-white font-bold">$1</b>')) }}></span>
                                                            </li>
                                                        );
                                                    })}
                                                </ul>
                                        ...
                                        // Paragraphs
                                        return (
                                            <p key={idx} className="text-sm leading-relaxed font-normal opacity-90 my-3" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(trimmedBlock.replace(/\*\*(.*?)\*\*/g, '<b class="text-white font-bold">$1</b>')) }}></p>
                                        );
                                    })}
                                </div>
                            ) : (
                        ...
                        <div className="mt-8 pt-6 border-t border-white/5 flex flex-col items-center gap-4">
                            ...
                            <button 
                                onClick={handleCloseModal}
                                className="w-full sm:w-auto px-10 py-3 bg-slate-800 hover:bg-slate-700 text-white font-bold text-xs uppercase tracking-widest rounded-radius-lg transition-all border border-white/5"
                            >
                                Close Window
                            </button>
                        </div>
