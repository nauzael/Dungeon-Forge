...
const InitiativeTracker: React.FC<InitiativeTrackerProps> = ({ partyMembers, combatants, onCombatantsChange, onSyncParty }) => {
  ...
  const dealDamage = (monsterId: string) => {
    const rawValue = damageInputs[monsterId];
    if (!rawValue) return;
    ...
    onCombatantsChange((prev) =>
      prev.map((combatant) => {
        if (combatant.id !== monsterId) return combatant;
        if (!combatant.hp) return combatant;
        return {
          ...combatant,
          hp: {
            ...combatant.hp,
            current: Math.max(0, combatant.hp.current - amount),
          },
        };
      })
    );
  ...
  const healMonster = (monsterId: string) => {
    const rawValue = damageInputs[monsterId];
    if (!rawValue) return;
    const amount = parseInt(rawValue, 10);
    if (Number.isNaN(amount) || amount <= 0) return;
    ...
    onCombatantsChange((prev) =>
      prev.map((combatant) => {
        ...
        return {
          ...
          hp: {
            ...combatant.hp,
            current: Math.min(combatant.hp.max, combatant.hp.current + amount),
          },
        };
      })
    );
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Add Combatant Form */}
      <div className="bg-white/5 rounded-radius-lg p-3 space-y-2">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          <input
            ...
            className="flex-1 min-w-[100px] px-3 py-2 rounded-radius-md text-sm outline-none focus:border-blue-500"
            style={{
          ...
          <input
            ...
            className="w-16 px-2 py-2 rounded-radius-md text-sm outline-none text-center focus:border-blue-500"
            style={{
              backgroundColor: 'var(--color-background-secondary)',
              borderColor: 'var(--color-border)',
              borderWidth: '1px',
          ...
          <input
            ...
            className="w-14 px-2 py-2 rounded-radius-md text-sm outline-none text-center focus:border-blue-500"
            style={{
              backgroundColor: 'var(--color-background-secondary)',
              borderColor: 'var(--color-border)',
              borderWidth: '1px',
          ...
          <input
            ...
            className="w-14 px-2 py-2 bg-white/10 rounded-radius-md text-white text-sm border border-white/10 focus:border-blue-500 outline-none text-center"
          />
          <button
            onClick={addCombatant}
            disabled={!newName.trim() || newInit === ''}
            className="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-600 disabled:cursor-not-allowed text-white rounded-radius-md text-sm font-bold flex items-center gap-1 transition-colors"
          >
      ...
      {/* Initiative List */}
      <div className="bg-black/20 rounded-radius-lg overflow-hidden">
        {/* Column Headers */}
        <div className="flex items-center gap-2 px-4 py-2 bg-white/5 text-[10px] text-slate-400 font-bold uppercase">
          ...
          <span className="min-w-[130px] text-center">HP</span>
              ...
              className={`flex items-center gap-2 px-4 py-3 border-b border-white/5 ${
                combatant.isCurrentTurn 
                  ...
                  : combatant.isPlayer 
                    ? 'bg-green-600/10' 
                    : 'bg-red-600/10'
              } ${!combatant.isPlayer && combatant.hp && combatant.hp.current <= 0 ? 'opacity-60' : ''}`}
            >
              ...
              <div className="flex-1 flex items-center gap-2">
                {combatant.isPlayer ? (
                  <span className="w-2 h-2 rounded-radius-pill bg-green-500"></span>
                ) : (
                  <span className="w-2 h-2 rounded-radius-pill bg-red-500"></span>
                )}
                <span className={`text-sm font-medium truncate ${!combatant.isPlayer && combatant.hp && combatant.hp.current <= 0 ? 'text-red-400 line-through' : 'text-white'}`}>
                  {!combatant.isPlayer && combatant.hp && combatant.hp.current <= 0 && '💀 '}{combatant.name}
                </span>
                {combatant.isCurrentTurn && (
                  <span className="text-[10px] bg-blue-500 text-white px-1.5 py-0.5 rounded-radius-sm font-bold">
                    TURN
                  </span>
                )}
              </div>
              ...
              <input
                ...
                className="w-12 px-2 py-1 bg-white/10 rounded-radius-sm text-white text-sm text-center border border-white/10 focus:border-blue-500 outline-none"
                placeholder="—"
              />
              ...
              <div className="flex items-center gap-1 min-w-[130px]">
                <span className={`w-14 text-center text-sm font-medium ${
                  ...
                  {combatant.hp ? (
                    <>{combatant.hp.current <= 0 ? '💀 ' : ''}{combatant.hp.current}/{combatant.hp.max}</>
                  ) : '—'}
                </span>

                {!combatant.isPlayer && combatant.hp && (
                  <div className="flex items-center gap-0.5">
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') dealDamage(combatant.id);
                      }}
                      placeholder="0"
                      className="w-10 px-1 py-0.5 bg-white/10 rounded-radius-sm text-white text-xs text-center border border-white/10 outline-none focus:border-blue-500"
                      onClick={(e) => e.stopPropagation()}
                    />
                    <button
                      onClick={() => dealDamage(combatant.id)}
                      className="w-6 h-6 flex items-center justify-center text-red-400 hover:text-red-300 bg-red-500/10 hover:bg-red-500/20 rounded-radius-sm transition-colors"
                      title="Deal damage"
                    >
                      <span className="material-symbols-outlined text-xs">remove</span>
                    </button>
                    <button
                      onClick={() => healMonster(combatant.id)}
                      className="w-6 h-6 flex items-center justify-center text-green-400 hover:text-green-300 bg-green-500/10 hover:bg-green-500/20 rounded-radius-sm transition-colors"
                      title="Heal"
                    >
              ...
              {combatants.filter(c => c.isPlayer && c.initiative === null).map(c => (
                <div key={c.id} className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    placeholder="Roll"
                    className="w-16 px-2 py-1 bg-white/10 rounded-radius-sm text-white text-sm text-center border border-white/10 focus:border-blue-500 outline-none"
                  />
                </div>
              ))}
            </div>
      ...
      <div className="flex gap-2">
        <button
          ...
          className="flex-1 py-3 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 disabled:cursor-not-allowed text-white rounded-radius-lg font-bold text-sm flex items-center justify-center gap-2 transition-colors"
        >
        ...
        <button
          onClick={resetCombat}
          className="px-4 py-3 bg-white/10 hover:bg-white/20 text-white rounded-radius-lg font-bold text-sm flex items-center gap-2 transition-colors"
        >
