...
const MemberCard: React.FC<MemberCardProps> = ({
  ...
  return (
    <div className="bg-[#1e293b] rounded-radius-xl p-4 border border-white/5 shadow-xl transition-all">
      {/* Member Header */}
      <div className="flex justify-between items-start mb-4">
        <div className="flex gap-3">
          <div className="size-12 rounded-radius-lg bg-slate-800 border border-white/10 overflow-hidden">
            {member.imageUrl && member.imageUrl !== 'DEFAULT' ? (
              <img
                src={member.imageUrl}
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="size-8 rounded-radius-md bg-red-500/10 border border-red-500/20 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all shadow-md shadow-red-900/10"
            title="Expulsar"
          >
      ...
      >
        ...
        <div className="h-3 w-full bg-slate-800 rounded-radius-pill overflow-hidden border border-white/5 relative">
          <div
            className="h-full bg-gradient-to-r from-red-500 to-emerald-500 transition-all duration-motion-hp"
            style={{
              width: `${
      ...
      <div className="flex justify-between items-center pt-3 border-t border-white/5">
        <div className="flex gap-1.5 overflow-x-auto scrollbar-none">
          {Object.entries(spellSlots).map(([lvl, info]) => (
            <div
              key={lvl}
              className="bg-blue-500/5 px-1.5 py-0.5 rounded-radius-sm border border-blue-500/10 flex items-center gap-1"
            >
              ...
              <div className="flex gap-[2px]">
                {Array.from({ length: info.max }).map((_, i) => (
                  <div
                    key={i}
                    className={`size-1 rounded-radius-pill ${
