...
const WildShapePanel: React.FC<WildShapePanelProps> = ({
  ...
  return (
    <div className="bg-card-dark rounded-radius-md border border-white/10 overflow-hidden">
      <div className="p-3">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            {isCircleOfTheMoon && (
              <span className="text-[10px] bg-moon/20 text-moon px-1.5 py-0.5 rounded-radius-pill">
                🌙 Luna
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-1">
            {Array.from({ length: character.wildShape?.max ?? 0 }).map((_, i) => (
              <span
                key={i}
                className={`w-2 h-2 rounded-radius-pill ${
        ...
        {!isTransformed ? (
          <button
            onClick={onTransform}
            disabled={!hasUses}
            className={`w-full py-2 px-3 rounded-radius-md text-sm font-bold transition-all ${
              hasUses
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 bg-primary/10 rounded-radius-md p-2 border border-primary/20">
              <span className="material-symbols-outlined text-primary text-xl">
            ...
            {beastAttacks.length > 0 && (
              <div className="rounded-radius-lg bg-white/5 p-3 border border-white/10 text-[10px] text-white/70 space-y-2">
            ...
            <button
              onClick={onRestore}
              className="w-full py-2 px-3 rounded-radius-md text-sm font-bold bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/30 transition-all"
            >
              Terminar Forma
            </button>
          </div>
