...
const BEAST_ICONS: Record<string, string> = {
  ...
  'Spider': 'bug_report',
  'Rat': 'pets',
  'Riding Horse': 'directions_car',
  'Warhorse': 'military_tech',
  'Boar': 'pets',
  'Giant Owl': 'nightlight_round',
  'Crocodile': 'water',
  'Brown Bear': 'pets',
  'Lion': 'pets',
  'Tiger': 'pets',
  'Giant Eagle': 'flight',
  'Giant Constrictor Snake': 'bug_report',
  'Giant Hyena': 'pets',
  'Giant Octopus': 'water',
};
...
const WildShapeModal: React.FC<WildShapeModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" style={{ height: '100dvh' }}>
      ...
      <div className="relative w-full max-w-sm max-h-[80dvh] mx-4 bg-background-dark border border-white/10 rounded-radius-xl overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-white/10">
          <div className="flex items-center gap-2">
            ...
            {isCircleOfTheMoon && (
              <span className="text-xs bg-moon/20 text-moon px-2 py-0.5 rounded-radius-pill">
                🌙 Circle of the Moon
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={toggleMode}
              className="px-3 py-1 text-xs bg-white/10 hover:bg-white/20 rounded-radius-md transition-colors"
            >
              {isManagingForms ? 'Transformar' : 'Gestionar'}
            </button>
            <button
              onClick={onClose}
              className="p-2 hover:bg-white/10 rounded-radius-md transition-colors"
            >
        ...
        <div className="p-3 bg-card-dark/50 border-b border-white/5">
          <div className="flex items-center justify-between text-xs text-white/60">
            {isManagingForms ? (
              <>
                <span>Known Forms: <span className="text-primary font-bold">{(character.wildShapeForms || []).length}/{maxKnownForms}</span></span>
                <span>Max CR: <span className="text-primary font-bold">{maxCR}</span></span>
              </>
            ) : (
              <>
                <span>Max CR: <span className="text-primary font-bold">{maxCR}</span></span>
                <span>THP on transform: <span className="text-green-400 font-bold">+{thp}</span></span>
              </>
            )}
          </div>
                ...
                {beasts.map((beast) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full p-3 rounded-radius-md border transition-all text-left ${
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                              {isManagingForms && (
                                <span className={`text-xs px-2 py-0.5 rounded-radius-pill ${
                                  isKnown 
        ...
        {selectedBeast && (
          <div className="p-4 border-t border-white/10 bg-card-dark/80">
            ...
            <button
              ...
              className="w-full py-3 px-4 rounded-radius-md bg-primary hover:bg-primary/80 disabled:bg-gray-500 disabled:cursor-not-allowed text-black font-bold transition-colors"
            >
              {isManagingForms ? (
