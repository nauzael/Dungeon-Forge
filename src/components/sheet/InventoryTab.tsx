...
import { Character, InventoryItem, ItemData, WeaponData, ArmorData, Ability, CustomItem } from '../../../types';
...
const InventoryTab: React.FC<InventoryTabProps> = ({ character, onUpdate, isReadOnly }) => {
    ...
    const [editingCustomItem, setEditingCustomItem] = useState<CustomItem | undefined>(undefined);
    ...
    const checkAttunement = (name: string) => {
        const data = getItemData(name, character);
    ...
    // Custom Item CRUD
    const addCustomItem = (customItem: CustomItem) => {
        const existing = character.customItems || [];
        // Check if name already exists
        if (existing.some(c => c.name === customItem.name)) {
            dialog.showAlert(`An item named "${customItem.name}" already exists.`);
            return;
        }
        const newCustomItems = [...existing, customItem];
        ...
        onUpdate({
            ...
            inventory: [newInventoryItem, ...(character.inventory || [])]
        });
        setShowCustomItemModal(false);
    };

    const updateCustomItem = (updatedItem: CustomItem) => {
        const existing = character.customItems || [];
        const oldName = editingCustomItem?.name;
        const newCustomItems = existing.map(c => c.name === oldName ? updatedItem : c);
        // Update inventory items if name changed
        let newInventory = character.inventory || [];
        if (oldName && oldName !== updatedItem.name) {
            newInventory = newInventory.map(i =>
                i.name === oldName ? { ...i, name: updatedItem.name } : i
            );
        }
        onUpdate({
            ...character,
    ...
    const deleteCustomItemDef = (itemName: string) => {
        const existing = character.customItems || [];
        // Remove from custom items and also remove from inventory
        const newInventory = (character.inventory || []).filter(i => i.name !== itemName);
        onUpdate({
            ...character,
            customItems: existing.filter(c => c.name !== itemName),
            inventory: newInventory,
        });
        setShowCustomItemModal(false);
        setEditingCustomItem(undefined);
    ...
    // Check if a given inventory item is a custom item
    const isCustomItem = (itemName: string) => {
        return character.customItems?.some(c => c.name === itemName) || false;
    };
    ...
    const toggleEquip = async (itemId: string) => {
        ...
        if (!itemToToggle.equipped && checkAttunement(itemToToggle.name)) {
            if (attunedCount >= 3) {
                await dialog.showAlert("Attunement Limit Reached! You can only attune to 3 magic items at a time. Unequip one before using this.");
                return;
            }
        }

        const itemData = getItemData(itemToToggle.name, character);
        ...
        const newInv = inventory.map(item => {
            ...
            if (isShield && !itemToToggle.equipped && item.equipped) {
                 const idata = getItemData(item.name, character);
    ...
    const totalWeight = inventory.reduce((acc, item) => {
        const itemData = getItemData(item.name, character);
    ...
    const renderItemRow = (item: InventoryItem) => {
        const itemData = getItemData(item.name, character) || { name: item.name, type: 'Gear', weight: 0, cost: '-', description: '' };
        ...
        return (
        <div key={item.id} onClick={() => setSelectedItem(item)} className="group relative flex items-center gap-4 p-3 rounded-radius-lg bg-surface-light dark:bg-surface-dark border border-gray-200 dark:border-white/5 shadow-sm active:scale-[0.99] transition-transform cursor-pointer">
            <div className={`size-12 rounded-radius-md flex items-center justify-center shrink-0 relative overflow-hidden ${item.equipped ? 'bg-primary text-white' : 'bg-gray-100 dark:bg-black/40 text-slate-600 dark:text-white'}`}>
            ...
            <div className="flex items-center gap-2">
                ...
                {ammoType && item.equipped && (
                    <span className={`text-[9px] font-black px-2 py-0.5 rounded-radius-pill ${ammoCount === 0 ? 'bg-red-500/20 text-red-600 dark:text-red-400' : ammoCount < 20 ? 'bg-yellow-500/20 text-yellow-600 dark:text-yellow-400' : 'bg-green-500/20 text-green-600 dark:text-green-400'}`}>
                        {ammoCount} {ammoType}
                    </span>
                )}
                {item.quantity > 1 && !ammoType && <span className="text-xs text-slate-500">x{item.quantity}</span>}
                {item.customStat && <span className="text-[10px] font-black text-primary bg-primary/10 px-1 rounded-radius-sm uppercase tracking-tighter">{item.customStat}</span>}
            </div>
            ...
            <div className="flex items-center gap-2">
            {!isReadOnly && isEquippable && (<button onClick={(e) => { e.stopPropagation(); toggleEquip(item.id); }} className={`px-3 py-1 rounded-radius-md text-[10px] font-bold uppercase tracking-wider border transition-colors ${item.equipped ? 'bg-primary/10 text-primary border-primary/20' : 'bg-slate-100 dark:bg-white/5 text-slate-500 border-transparent'}`}>{item.equipped ? 'Equipped' : 'Equip'}</button>)}
    ...
    const renderItemDetail = (data: ItemData, onAction?: () => void, actionLabel?: string, isPreview: boolean = false, onEdit?: () => void) => {
               ...
               <div className="flex items-start justify-between mb-4">
                   <div className="min-w-0 pr-4">
                       ...
                       <div className="flex flex-wrap gap-2 mt-1">
                           <span className="text-[10px] font-bold text-primary uppercase tracking-widest bg-primary/5 px-2 py-0.5 rounded-radius-sm">{data.type}</span>
                           {requiresAttunement && <span className="text-[10px] font-bold text-purple-500 uppercase tracking-widest bg-purple-500/10 px-2 py-0.5 rounded-radius-sm flex items-center gap-1"><span className="material-symbols-outlined text-[10px]">auto_awesome</span> Attunement</span>}
                       </div>
                   </div>
                   <button onClick={() => isPreview ? setPreviewItemName(null) : setSelectedItem(null)} className="w-8 h-8 flex items-center justify-center rounded-radius-pill bg-slate-100 dark:bg-white/5 text-slate-400 hover:text-slate-600">
               ...
               <div className="flex-1 overflow-y-auto space-y-3 mb-6 no-scrollbar pr-1 border-t border-slate-100 dark:border-white/5 pt-4">
                   ...
                   {isPreview && isEquippableType && (
                       <div className="py-2 border-b border-slate-100 dark:border-white/5">
                           ...
                           <div className="flex bg-slate-100 dark:bg-white/5 p-1 rounded-radius-lg">
                               {[0, 1, 2, 3].map(lvl => (
                                   <button 
                                       key={lvl}
                                       onClick={() => setEnchantmentLevel(lvl)}
                                       className={`flex-1 py-1.5 rounded-radius-md text-xs font-bold transition-all ${enchantmentLevel === lvl ? 'bg-white dark:bg-surface-dark shadow-elev-raised text-primary' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                                   >
                                       {lvl === 0 ? 'Normal' : `+${lvl}`}
                                   </button>
                               ))}
                   ...
                   {isPreview && data.type === 'Weapon' && (
                       <div className="py-2 border-b border-slate-100 dark:border-white/5">
                            ...
                            <div className="grid grid-cols-3 gap-1 bg-slate-100 dark:bg-white/5 p-1 rounded-radius-lg">
                               {(['Default', 'STR', 'DEX', 'INT', 'WIS', 'CHA'] as const).map(stat => (
                                    <button 
                                        key={stat}
                                        onClick={() => setSelectedStat(stat as Ability | 'Default')}
                                       className={`py-1.5 rounded-radius-md text-[10px] font-black transition-all ${selectedStat === stat ? 'bg-white dark:bg-surface-dark shadow-elev-raised text-primary' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                                   >
                                       {stat}
                                   </button>
                               ))}
                ...
                <div className={`grid ${isPreview ? 'grid-cols-1' : onEdit ? 'grid-cols-3' : 'grid-cols-2'} gap-3 shrink-0 pt-2`}>
                    {onEdit && (
                        <button 
                            onClick={onEdit} 
                            className="py-3.5 rounded-radius-xl font-bold text-sm bg-amber-500/10 text-amber-500 border border-amber-500/20 hover:bg-amber-500 hover:text-white transition-all"
                        >
                            Edit
                        </button>
                    )}
                    {onAction && (
                        <button 
                            onClick={onAction} 
                            className="py-3.5 rounded-radius-xl font-bold text-sm bg-primary text-background-dark shadow-elev-modal shadow-primary/20 active:scale-[0.97] transition-all"
                        >
                            {actionLabel}
                        </button>
                    )}
                    {!isPreview && (
                        <button 
                            ...
                            className="py-3.5 rounded-radius-xl font-bold text-sm bg-red-500/10 text-red-500 border border-red-500/20 hover:bg-red-500 hover:text-white transition-all"
                         >
                             Delete
                         </button>
                    )}
       ...
       <div className="pt-4 shrink-0 flex flex-col gap-4">
           ...
           <div className="grid grid-cols-2 gap-3">
               <div className="flex flex-col gap-2">
                  ...
                  <div className="h-1.5 w-full bg-gray-200 dark:bg-surface-dark rounded-radius-pill overflow-hidden">
                     <div className={`h-full rounded-radius-pill ${totalWeight > carryCap ? 'bg-red-500' : 'bg-primary'}`} style={{ width: `${Math.min(100, (totalWeight / carryCap) * 100)}%` }}></div>
                  </div>
               </div>
               <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-end">
                  ...
                  <div className="h-1.5 w-full bg-gray-200 dark:bg-surface-dark rounded-radius-pill overflow-hidden">
                     <div className={`h-full rounded-radius-pill ${attunedCount >= 3 ? 'bg-amber-500' : 'bg-purple-500'}`} style={{ width: `${Math.min(100, (attunedCount / 3) * 100)}%` }}></div>
                  </div>
               </div>
           </div>
           ...
           {!isReadOnly && (
               <button 
                    ...
                    className="flex items-center justify-center gap-2 w-full py-3 bg-surface-light dark:bg-white/5 border border-dashed border-slate-300 dark:border-white/10 rounded-radius-lg text-primary font-bold hover:bg-primary/5 dark:hover:bg-white/10 transition-colors mb-2"
               >
           ...
           <div className="p-4 rounded-radius-2xl bg-white dark:bg-surface-dark border border-slate-200 dark:border-white/5 shadow-sm">
           ...
           <TabletPotionBelt isTablet={isTablet}>
               {!isReadOnly && (
                   <button 
                       ...
                       className="flex flex-col items-center justify-center gap-2 min-w-[100px] p-3 rounded-radius-xl border-2 border-dashed border-slate-200 dark:border-white/5 text-slate-300 dark:text-slate-600 hover:text-primary hover:border-primary transition-all shrink-0"
                   >
                ...
                {quickPotions.map(potion => {
                    const itemData = getItemData(potion.name, character);
                   return (
                   <div 
                       key={potion.id}
                       className="flex flex-col gap-2 min-w-[140px] p-3 rounded-radius-xl bg-white dark:bg-surface-dark border border-slate-200 dark:border-white/5 shadow-sm active:scale-95 transition-transform"
                   >
                       <div className="flex items-start gap-2">
                           <div className="size-8 rounded-radius-md bg-pink-500/10 text-pink-500 flex items-center justify-center shrink-0">
                       ...
                       {!isReadOnly && (
                           <button
                               ...
                               className="w-full py-1.5 rounded-radius-md bg-primary/10 hover:bg-primary text-primary hover:text-white text-[10px] font-black uppercase tracking-widest transition-all"
                           >
                               Drink
                           </button>
                       )}
...
{quickPotions.length === 0 && !isReadOnly && (
               <p className="text-[10px] italic text-slate-400 px-1">Your belt is empty. Add potions to see them here.</p>
           )}
       </div>
       ...
       <TabletInventoryGrid
           ...
           emptyMessage={<div className="p-4 text-center border border-dashed border-slate-200 dark:border-white/5 rounded-radius-lg"><p className="text-sm text-slate-400 italic">Your backpack is empty.</p></div>}
       />
       ...
       {showAddItem && createPortal(
           <div className="fixed inset-0 z-50 bg-background-light dark:bg-background-dark flex flex-col p-4 animate-fadeIn pt-[max(0.75rem,env(safe-area-inset-top))]">
               <div className="flex items-center gap-3 mb-4">
                   <button onClick={() => setShowAddItem(false)} className="size-10 rounded-radius-pill bg-slate-100 dark:bg-white/10 flex items-center justify-center"><span className="material-symbols-outlined">close</span></button>
                    <div className="flex-1 relative"><input autoFocus type="text" placeholder="Search items..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-slate-100 dark:bg-black/20 border-none rounded-radius-lg py-2.5 pl-10 pr-4 outline-none focus:ring-2 ring-primary/50 dark:text-white"/><span className="material-symbols-outlined absolute left-3 top-2.5 text-slate-400">search</span></div>
                </div>
                <div className="flex-1 overflow-y-auto no-scrollbar space-y-2">
                    {/* Create Custom Item button */}
                    <button
                        onClick={() => { setEditingCustomItem(undefined); setShowCustomItemModal(true); }}
                        className="w-full text-left p-3 rounded-radius-lg bg-primary/5 hover:bg-primary/10 border-2 border-dashed border-primary/30 hover:border-primary/50 flex items-center justify-center gap-2 transition-all mb-3"
                    >
                        <span className="material-symbols-outlined text-primary text-lg">add_circle</span>
                        <span className="font-bold text-primary text-sm">Create Custom Item</span>
                    </button>
                    {Object.keys(ALL_ITEMS).filter(name => name.toLowerCase().includes(searchQuery.toLowerCase())).slice(0, 50).map(name => {
                           ...
                           return (
                               <button 
                                    ...
                                    className="w-full text-left p-3 rounded-radius-lg hover:bg-slate-100 dark:hover:bg-white/5 flex justify-between items-center border border-transparent hover:border-slate-200 dark:hover:border-white/10 transition-all"
                                >
                                    <div>
       ...
       {selectedItem && createPortal(
         <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeIn" onClick={() => setSelectedItem(null)}>
            <div className="w-full max-w-sm bg-white dark:bg-surface-dark rounded-radius-2xl p-6 shadow-2xl animate-scaleUp flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
                {(() => {
                    const itemData = (getItemData(selectedItem.name, character) || { name: selectedItem.name, type: 'Gear', weight: 0, cost: '-', description: '' }) as ItemData;
                    const isMagic = MAGIC_ITEMS[selectedItem.name] !== undefined || selectedItem.name.match(/\+(\d+)$/) !== null;
                    const isCustom = isCustomItem(selectedItem.name);
                    const isEquippable = itemData.type === 'Weapon' || itemData.type === 'Armor' || isMagic;
                    ...
                    return renderItemDetail(
                        ...
                        false,
                        isCustom ? () => {
                            const customData = character.customItems?.find(c => c.name === selectedItem.name);
                            if (customData) {
                                setEditingCustomItem(customData);
        ...
        {showCustomItemModal && (
            <CustomItemModal
                ...
                onSave={editingCustomItem ? updateCustomItem : addCustomItem}
                onClose={() => { setShowCustomItemModal(false); setEditingCustomItem(undefined); }}
                onDelete={editingCustomItem ? deleteCustomItemDef : undefined}
            />
        )}
