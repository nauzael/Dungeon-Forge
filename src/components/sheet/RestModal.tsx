...
const RestModal: React.FC<RestModalProps> = ({ character, onComplete, onCancel }) => {
    ...
    const handleShortRest = () => {
        ...
        if (charClass === 'Monk' || charClass === 'Pugilist') updated.focus = { current: updated.level, max: updated.level };
    ...
    const handleLongRest = () => {
        ...
        // Restore spell slots for all caster types
        const FULL_CASTER = ['Bard', 'Cleric', 'Druid', 'Sorcerer', 'Wizard'];
        const HALF_CASTER = ['Paladin', 'Ranger'];
        const THIRD_CASTER = ['Fighter', 'Rogue']; // Eldritch Knight, Arcane Trickster (subclass)

        if (FULL_CASTER.includes(charClass) || HALF_CASTER.includes(charClass) || THIRD_CASTER.includes(charClass)) {
            updated.usedSlots = {};
            if (updated.spellSlots) {
                const restoredSlots: Record<number, { current: number; max: number }> = {};
                for (const [level, slot] of Object.entries(updated.spellSlots)) {
                    restoredSlots[Number(level)] = { current: slot.max, max: slot.max };
                }
                updated.spellSlots = restoredSlots;
            }
        }

        if (charClass === 'Warlock') {
            const pactLevel = updated.pactSlotLevel || 1;
            const pactCount = updated.level >= 17 ? 4 : updated.level >= 11 ? 3 : updated.level >= 2 ? 2 : 1;
            updated.usedSlots = {};
            updated.spellSlots = {
                ...(updated.spellSlots || {}),
                [pactLevel]: { current: pactCount, max: pactCount }
            };
        }

        // Wizard: On long rest, the player may change prepared spells.
        // We DO NOT clear preparedSpells here to avoid data loss.
        // The character retains previous selections as the default next-day set.
        if (charClass === 'Wizard') {
            updated.arcaneRecovery = { uses: 1 };
        }
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/80 dark:bg-black/60 flex items-end sm:items-center justify-center animate-fadeIn backdrop-blur-sm">
            <div className="w-full sm:max-w-lg bg-white dark:bg-[#0f1525] rounded-t-3xl sm:rounded-radius-2xl shadow-2xl max-h-[95vh] sm:max-h-[85vh] flex flex-col animate-slideUp overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-white/10 shrink-0">
                    <button onClick={onCancel} className="w-10 h-10 flex items-center justify-center rounded-radius-lg hover:bg-slate-100 dark:hover:bg-white/10 transition-colors">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    {!restType && (
                        <>
                            <button onClick={() => setRestType('short')} className="w-full p-4 rounded-radius-xl border-2 border-amber-500/20 bg-amber-500/5 hover:bg-amber-500/10 transition-all text-left">
                            ...
                            <button onClick={() => canDoLongRest && setRestType('long')} disabled={!canDoLongRest} className={`w-full p-4 rounded-radius-xl border-2 transition-all text-left ${canDoLongRest ? 'border-emerald-500/20 bg-emerald-500/5 hover:bg-emerald-500/10' : 'border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 opacity-50 cursor-not-allowed'}`}>
                                <div className="flex items-center gap-3 mb-2">
                    ...
                    {restType === 'short' && (
                        <div className="space-y-4">
                            ...
                            <div className="p-4 rounded-radius-xl bg-amber-500/5 border border-amber-500/20">
                                ...
                                {spentHitDice > 0 ? (
                                    <div className="mb-4">
                                        ...
                                        <input type="range" min={0} max={spentHitDice} value={diceToSpend} onChange={(e) => setDiceToSpend(parseInt(e.target.value))} className="w-full h-2 bg-slate-200 dark:bg-white/10 rounded-radius-pill appearance-none cursor-pointer accent-amber-500" />
                                        ...
                                        {diceToSpend > 0 && (
                                            <div className="mt-2 p-2 rounded-radius-lg bg-amber-500/10 text-sm text-slate-700 dark:text-slate-300">
                                                HP Recovery: +{diceToSpend * hpRecoveryPerDie} ({diceToSpend}d{dieType} + {conMod} CON)
                                            </div>
                                        )}
                                        {character.class === 'Barbarian' && (
                                            <div className="mt-2 p-2 rounded-radius-lg bg-red-500/10 border border-red-500/20 text-sm text-slate-700 dark:text-slate-300">
                                                🔥 Rage Recovery: +1 charge
                                            </div>
                                        )}
                                    </div>
                                ) : (
                                    <div className="mb-4 p-3 rounded-radius-lg bg-slate-100 dark:bg-white/5 text-sm text-slate-500">No Hit Dice spent</div>
                                )}
                            </div>
                            
                            <button onClick={handleShortRest} disabled={isProcessing} className="w-full py-3 rounded-radius-lg font-bold text-white bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 disabled:opacity-50 transition-all">
                    ...
                    {restType === 'long' && (
                        <div className="space-y-4">
                            ...
                            <div className="p-4 rounded-radius-xl bg-emerald-500/5 border border-emerald-500/20">
                                <div className="flex items-center gap-2 mb-3">
                            ...
                            <button onClick={handleLongRest} disabled={isProcessing || !canDoLongRest} className="w-full py-3 rounded-radius-lg font-bold text-white bg-gradient-to-r from-emerald-500 to-green-500 hover:from-emerald-600 hover:to-green-600 disabled:opacity-50 transition-all">
