    ...
    return (
        <div className="flex flex-col h-full">
            <div className="text-center px-4 py-3 pb-2 shrink-0">
                <div className="w-12 h-12 rounded-radius-lg bg-gradient-to-br from-purple-100 to-pink-100 dark:from-purple-900/40 dark:to-pink-900/40 text-purple-600 dark:text-purple-400 flex items-center justify-center mx-auto mb-2 border border-white dark:border-white/20 shadow-sm">
            ...
            >
                <div className="flex-1">
                    <div className="bg-white dark:bg-[#1a2235] rounded-radius-xl border border-slate-200 dark:border-white/10 overflow-hidden shadow-xl">
                            ...
                            <div className="border-t border-slate-100 dark:border-white/5 pt-2">
                                ...
                                <div className="space-y-2">
                                    {level3Features.map((feature: any, idx: number) => (
                                        <div key={idx} className="bg-slate-50 dark:bg-white/5 rounded-radius-lg p-2.5">
                                ...
                                <div className="border-t border-slate-100 dark:border-white/5 pt-2">
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                        {alwaysPrepared.map((spell: string, idx: number) => (
                                            <span
                                                key={idx}
                                                className="text-[9px] font-semibold px-2 py-0.5 bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded-radius-md"
                                            >
                                                {spell}
                                            </span>
                                        ))}
                                    ...
                                    {allFeatures.filter((f: any) => f.level > 3).map((feature: any, idx: number) => (
                                        <div key={idx} className="bg-slate-50 dark:bg-white/5 rounded-radius-lg p-2.5">
                                            <div className="flex items-center gap-1.5 mb-1">
                                                <span className="text-[8px] font-black uppercase px-1.5 py-0.5 bg-slate-200 dark:bg-white/10 text-slate-600 dark:text-slate-300 rounded-radius-sm">
                                                    Lv{feature.level}
                                                </span>
            ...
            <div className="flex items-center justify-center gap-1.5 py-2 px-3 shrink-0">
                {subclasses.map((_: any, idx: number) => (
                    <button
                        ...
                        className={`h-2 rounded-radius-pill transition-all duration-motion-base ${
                            idx === currentIndex
            ...
            <div className="p-3 pt-0 shrink-0">
                <button
                    ...
                    className="w-full py-3 rounded-radius-lg bg-gradient-to-r from-purple-500 to-pink-500 text-white font-bold text-xs shadow-md hover:shadow-elev-modal hover:from-purple-600 hover:to-pink-600 transition-all active:scale-[0.98]"
                >
