    ...
    return (
        <div className="flex flex-col h-full p-4">
            <div className="text-center mb-4 shrink-0">
                <div className="w-14 h-14 rounded-radius-pill flex items-center justify-center mx-auto mb-2 border-4 border-white dark:border-surface-dark shadow-elev-modal bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400">
            ...
            <div className="flex gap-2 mb-3 overflow-x-auto pb-1 shrink-0 no-scrollbar">
                {levelFilters.map(level => (
                    <button
                        ...
                        className={`px-3 py-1 rounded-radius-pill text-xs font-bold whitespace-nowrap transition-all ${
                            filterLevel === level
                ...
                {filteredSpells.map(({ name, detail }) => {
                    ...
                    return (
                        <button
                            ...
                            className={`w-full p-3 rounded-radius-lg border text-left transition-all ${
                                isSelected
                        ...
                        >
                            <div className="flex justify-between items-center mb-1">
                                <div className="flex items-center gap-2">
                                    ...
                                    {detail.level === 0 ? (
                                        <span className="text-[10px] font-semibold px-1.5 py-0.5 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded-radius-sm">
                                            Cantrip
                                        </span>
                                    ) : (
                                        <span className="text-[10px] font-semibold px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-radius-sm">
                                            Lv {detail.level}
                                        </span>
                                    )}
                                    {isAlreadyKnown && (
                                        <span className="text-[10px] font-semibold px-1.5 py-0.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-radius-sm">
                                            Known
                                        </span>
                                    )}
                                </div>
            ...
            {selectedSpells.length > 0 && (
                <div className="mt-3 p-3 bg-purple-500/10 rounded-radius-lg border border-purple-500/20 shrink-0">
