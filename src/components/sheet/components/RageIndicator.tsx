    ...
    return (
        <>
            <div className="flex flex-col gap-3">
                <div className="relative mt-[5px] mb-[5px]">
                    <button
                        ...
                        className={`
                            w-full flex items-center justify-between gap-3 px-4 py-3 rounded-radius-xl 
                            transition-all duration-motion-base transform
                            ${state.isRaging 
                                ? 'bg-gradient-to-r from-red-600 to-red-500 text-white shadow-elev-modal shadow-red-500/30 animate-pulse' 
                    ...
                    {state.isRaging && (
                        <div className="flex flex-wrap gap-2 mt-2">
                            {rageDamage > 0 && (
                                <div className="flex items-center gap-1 px-2 py-1 rounded-radius-md bg-red-500/10 text-red-600 dark:text-red-400 text-xs">
                                    <span className="material-symbols-outlined text-sm">local_fire_department</span>
                                    +{rageDamage} damage
                                </div>
                            )}
                            <div className="flex items-center gap-1 px-2 py-1 rounded-radius-md bg-blue-500/10 text-blue-600 dark:text-blue-400 text-xs">
                                <span className="material-symbols-outlined text-sm">shield</span>
                                B/P/S Resist
                            </div>
                            <div className="flex items-center gap-1 px-2 py-1 rounded-radius-md bg-amber-500/10 text-amber-600 dark:text-amber-400 text-xs">
                                <span className="material-symbols-outlined text-sm">fitness_center</span>
                                STR Adv
                            </div>
                            {character.subclass === 'Path of the World Tree' && worldTreeHP > 0 && (
                                <div className="flex items-center gap-1 px-2 py-1 rounded-radius-md bg-green-500/10 text-green-600 dark:text-green-400 text-xs">
                                    <span className="material-symbols-outlined text-sm">forest</span>
                                    +{worldTreeHP} temp HP
                                </div>
                            )}
                            {character.subclass === 'Path of the Zealot' && divineFuryDamage > 0 && (
                                <div className="flex items-center gap-1 px-2 py-1 rounded-radius-md bg-yellow-500/10 text-yellow-600 dark:text-yellow-400 text-xs">
                                    <span className="material-symbols-outlined text-sm">bolt</span>
                                    Divine +{divineFuryDamage}
                                </div>
                            )}
                            {character.subclass === 'Path of the Berserker' && (
                                <div className="flex items-center gap-1 px-2 py-1 rounded-radius-md bg-purple-500/10 text-purple-600 dark:text-purple-400 text-xs">
                ...
                {character.subclass === 'Path of the Berserker' && (
                    <button
                        ...
                        disabled={isReadOnly}
                        className={`
                            w-full flex items-center justify-between gap-3 px-4 py-2 rounded-radius-lg 
                    ...
                    >
                        ...
                        <div className="flex items-center gap-1">
                            <span className={`w-2 h-2 rounded-radius-pill ${state.isRecklessAttack ? 'bg-yellow-500' : 'bg-slate-400'}`} />
            ...
            {showConfirmModal && (
                ...
                >
                    <div className="bg-white dark:bg-[#1a2235] rounded-radius-xl p-5 shadow-2xl animate-scaleIn w-full max-w-[320px]" onClick={(e) => e.stopPropagation()}>
                        <div className="flex flex-col items-center text-center">
                            ...
                            <div className="flex gap-3 w-full">
                                <button
                                    onClick={cancelAction}
                                    className="flex-1 py-2.5 px-4 rounded-radius-lg bg-slate-100 dark:bg-white/10 text-slate-700 dark:text-slate-300 font-medium hover:bg-slate-200 dark:hover:bg-white/20 transition-colors"
                                >
                                    Cancel
                                </button>
                                <button
                                    onClick={confirmAction}
                                    className={`flex-1 py-2.5 px-4 rounded-radius-lg font-medium transition-colors ${
                                        showConfirmModal === 'activate'
