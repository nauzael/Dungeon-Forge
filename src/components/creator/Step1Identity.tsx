...
const Step1Identity: React.FC<Step1Props> = ({
    ...
    const bgMagicConfig = useMemo(() => {
        ...
        if (backgroundData) {
            ...
            const isMagicInitiate = featName.includes('Magic Initiate');
            
            if (isMagicInitiate) {
                let listType: 'Cleric' | 'Druid' | 'Wizard' = 'Wizard';
                if (featName.includes('Cleric')) listType = 'Cleric';
                if (featName.includes('Druid')) listType = 'Druid';
        ...
        if (selectedSpecies === 'Human' && selectedFeat.includes('Magic Initiate')) {
            let listType: 'Cleric' | 'Druid' | 'Wizard' = 'Wizard';
            if (selectedFeat.includes('Cleric')) listType = 'Cleric';
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.size > 2 * 1024 * 1024) {
            void dialog.showAlert("The image is too large. Please choose one under 2MB.");
            return;
        }
    ...
    const handleUrlPrompt = () => {
        setUrlInput(charImage === DEFAULT_CHAR_IMAGE ? '' : charImage);
        ...
        <>
            ...
            <div className="px-6 mb-4 flex items-center gap-4">
                <div className="relative shrink-0 group cursor-pointer" onClick={() => setShowImageOptions(true)}>
                    <div className="w-20 h-20 rounded-radius-xl bg-surface-dark border-2 border-dashed border-slate-600 flex items-center justify-center overflow-hidden hover:border-primary transition-all bg-cover bg-center" style={charImage !== DEFAULT_CHAR_IMAGE ? { backgroundImage: `url(${charImage})`, borderStyle: 'solid' } : {}}>
                    ...
                    <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-primary rounded-radius-pill flex items-center justify-center border-2 border-background-dark">
                ...
                <div className="flex-1">
                    ...
                    <input value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-white dark:bg-surface-dark border border-slate-200 dark:border-white/10 rounded-radius-lg px-4 py-3 text-lg font-medium placeholder:text-slate-400 focus:ring-2 focus:ring-primary/50 outline-none transition-all shadow-sm" placeholder={t.character_name_placeholder} type="text" />
                </div>
            </div>
            ...
            {showImageOptions && createPortal(
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeIn" onClick={() => setShowImageOptions(false)}>
                    <div className="w-full max-w-sm bg-white dark:bg-surface-dark rounded-radius-2xl p-6 shadow-2xl animate-scaleUp" onClick={e => e.stopPropagation()}>
                        <h3 className="text-xl font-bold mb-6 text-center">{t.profile_image}</h3>
                        <div className="grid grid-cols-1 gap-3">
                            <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-4 p-4 rounded-radius-xl bg-primary text-background-dark font-bold transition-transform active:scale-95">
                                <span className="material-symbols-outlined">upload</span>
                                <span>{t.upload_local}</span>
                            </button>
                            <button onClick={handleUrlPrompt} className="flex items-center gap-4 p-4 rounded-radius-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 font-bold transition-transform active:scale-95">
                                <span className="material-symbols-outlined">link</span>
                                <span>{t.use_url}</span>
                            </button>
                        </div>
                        {charImage !== DEFAULT_CHAR_IMAGE && (
                            <button onClick={() => { setCharImage(DEFAULT_CHAR_IMAGE); setShowImageOptions(false); }} className="flex items-center gap-4 p-4 rounded-radius-xl bg-red-500/10 text-red-500 border border-red-500/20 font-bold transition-transform active:scale-95">
                                ...
                                {[
                                    ...
                                    .map((url, idx) => (
                                        <button 
                                            ...
                                            className={`aspect-square rounded-radius-lg bg-cover bg-center border-2 transition-all active:scale-90 ${charImage === url ? 'border-primary' : 'border-transparent'}`}
            ...
            <div className="mb-4">
                        ...
                        {CLASS_LIST.map((cls) => {
                            ...
                            return (
                                <label key={cls} className="relative shrink-0 cursor-pointer group/card snap-center scroll-m-6">
                                    ...
                                    <div className={`w-36 h-52 rounded-radius-2xl p-4 flex flex-col items-center justify-between transition-all duration-motion-base ease-out border-2 ${isSelected ? 'border-primary shadow-[0_10px_30px_-10px_rgba(53,158,255,0.4)] scale-105' : 'border-transparent shadow-sm'}`} style={isSelected ? {} : {backgroundColor: 'var(--color-surface)', opacity: '0.7'}}>
                                        <div className={`w-16 h-16 rounded-radius-xl flex items-center justify-center transition-all duration-motion-base shadow-inner ${isSelected ? 'bg-primary/10 rotate-3' : 'group-hover/card:scale-110'}`} style={isSelected ? {} : {backgroundColor: 'var(--color-background-secondary)'}}>
                                        ...
                                        <div className={`absolute top-3 right-3 w-6 h-6 rounded-radius-pill bg-primary flex items-center justify-center text-black shadow-sm transition-all duration-motion-base ${isSelected ? 'opacity-100 scale-100 rotate-0' : 'opacity-0 scale-50 -rotate-90'}`}>
                ...
                <div className="px-6 mt-2 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="p-3 rounded-radius-lg border" style={{backgroundColor: 'var(--color-surface)', borderColor: 'var(--color-border)'}}>
                ...
                <div className="px-6 mt-4">
                    ...
                    <div className="relative">
                        <select value={level} onChange={(e) => setLevel(Number(e.target.value))} className="w-full bg-white dark:bg-surface-dark border border-slate-200 dark:border-white/10 rounded-radius-lg pl-4 pr-10 py-3 text-base font-bold text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-primary/50 transition-all shadow-sm appearance-none">
                            ...
                            {availableSubclasses.map((sub) => {
                                ...
                                return (
                                    <label key={sub.name} className="relative shrink-0 cursor-pointer group/card snap-center scroll-m-6">
                                        ...
                                        <div className={`w-64 min-h-[14rem] h-full rounded-radius-xl p-5 flex flex-col items-center gap-1 transition-all duration-motion-base ease-out border-2 text-center relative ${isSelected ? 'border-primary shadow-[0_10px_30px_-10px_rgba(53,158,255,0.4)] scale-105 z-10' : 'border-transparent shadow-sm'}`} style={isSelected ? {} : {backgroundColor: 'var(--color-surface)', opacity: '0.7'}}>
                                            ...
                                            <div className={`absolute top-2 right-2 w-5 h-5 rounded-radius-pill bg-primary flex items-center justify-center text-black shadow-sm transition-all duration-motion-base ${isSelected ? 'opacity-100 scale-100 rotate-0' : 'opacity-0 scale-50 -rotate-90'}`}>
                        ...
                        {SPECIES_LIST.map((s) => {
                            ...
                            return (
                                <label key={s} className="relative shrink-0 cursor-pointer group/card snap-center scroll-m-6">
                                    ...
                                    <div className={`w-28 h-32 rounded-radius-xl p-3 flex flex-col items-center justify-center gap-2 transition-all duration-motion-base ease-out border-2 ${isSelected ? 'border-primary shadow-elev-modal scale-105' : 'border-transparent'}`} style={isSelected ? {} : {backgroundColor: 'var(--color-surface)', opacity: '0.7'}}>
                                        <div className={`w-10 h-10 rounded-radius-lg flex items-center justify-center transition-all duration-motion-base ${isSelected ? 'bg-primary/10' : ''}`} style={isSelected ? {} : {backgroundColor: 'var(--color-background-secondary)'}}>
                                        ...
                                        <div className={`absolute top-2 right-2 w-4 h-4 rounded-radius-pill bg-primary flex items-center justify-center text-black shadow-sm transition-all duration-motion-base ${isSelected ? 'opacity-100 scale-100' : 'opacity-0 scale-50'}`}>
                            ...
                            {availableSubspecies.map((sub: any) => {
                                ...
                                return (
                                    <label key={sub.name} className="relative shrink-0 cursor-pointer group/card snap-center scroll-m-6">
                                        ...
                                        <div className={`w-64 min-h-[12rem] h-full rounded-radius-xl p-5 flex flex-col items-center gap-1 transition-all duration-motion-base ease-out border-2 text-center relative ${isSelected ? 'border-primary shadow-elev-modal scale-105 z-10' : 'border-transparent shadow-sm'}`} style={isSelected ? {} : {backgroundColor: 'var(--color-surface)', opacity: '0.7'}}>
                                            <div className="flex flex-col items-center w-full gap-2 flex-1">
                                                <span className={`block font-bold text-sm leading-tight px-2 ${isSelected ? 'text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-300'}`}>{sub.name}</span>
                                                <div className="w-full border-t border-slate-100 dark:border-white/5 my-0.5 opacity-50"></div>
                                                <span className="block text-xs text-slate-500 dark:text-slate-400 leading-relaxed px-1 italic">{sub.description}</span>
            ...
            <div className="mb-4">
                        ...
                        {Object.keys(backgrounds).map((b) => {
                            ...
                            return (
                                <label key={b} className="relative shrink-0 cursor-pointer group/card snap-center scroll-m-6">
                                    ...
                                    <div className={`w-36 h-28 rounded-radius-xl p-3 flex flex-col items-center justify-center gap-1 transition-all duration-motion-base ease-out border-2 ${isSelected ? 'border-primary shadow-elev-modal scale-105' : 'border-transparent'}`} style={isSelected ? {} : {backgroundColor: 'var(--color-surface)', opacity: '0.7'}}>
                ...
                <div className="px-6 mt-2">
                    <div className="p-5 bg-white dark:bg-surface-dark border border-slate-200 dark:border-white/10 rounded-radius-xl shadow-sm">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-radius-lg bg-blue-500/10 flex items-center justify-center text-blue-500 shrink-0">
                        ...
                        {/* BACKGROUND MAGIC SETUP - MODAL BUTTON */}
                        {bgMagicConfig && (
                            <div className="mt-4 pt-4 border-t border-slate-100 dark:border-white/5 space-y-3 animate-fadeIn">
                                <button 
                                    onClick={() => setShowMagicModal(true)}
                                    className={`w-full flex items-center justify-between p-4 rounded-radius-xl border-2 transition-all group ${bgSpells.length === (bgMagicConfig.cantripsNeeded + bgMagicConfig.level1Needed) ? 'bg-primary/5 border-primary/40' : 'bg-slate-50 dark:bg-black/20 border-slate-200 dark:border-white/5 hover:border-primary/50'}`}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`size-10 rounded-radius-lg flex items-center justify-center transition-colors ${bgSpells.length === (bgMagicConfig.cantripsNeeded + bgMagicConfig.level1Needed) ? 'bg-primary text-white' : 'bg-primary/10 text-primary group-hover:bg-primary group-hover:text-white'}`}>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className={`text-xs font-black px-2 py-1 rounded-radius-md ${bgSpells.length === (bgMagicConfig.cantripsNeeded + bgMagicConfig.level1Needed) ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-200 dark:bg-white/10 text-slate-500'}`}>
                        ...
                        {bgSkilledConfig && (
                            <div className="mt-4 pt-4 border-t border-slate-100 dark:border-white/5 space-y-3 animate-fadeIn">
                                <button 
                                    ...
                                    className={`w-full flex items-center justify-between p-4 rounded-radius-xl border-2 transition-all group ${bgSkilledSkills.length === bgSkilledConfig.skillsNeeded ? 'bg-primary/5 border-primary/40' : 'bg-slate-50 dark:bg-black/20 border-slate-200 dark:border-white/5 hover:border-primary/50'}`}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`size-10 rounded-radius-lg flex items-center justify-center transition-colors ${bgSkilledSkills.length === bgSkilledConfig.skillsNeeded ? 'bg-primary text-white' : 'bg-primary/10 text-primary group-hover:bg-primary group-hover:text-white'}`}>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className={`text-xs font-black px-2 py-1 rounded-radius-md ${bgSkilledSkills.length === bgSkilledConfig.skillsNeeded ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-200 dark:bg-white/10 text-slate-500'}`}>
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-100 dark:border-white/5 space-y-4">
                            ...
                            <div className="flex items-start gap-3 bg-primary/5 border border-primary/10 p-3 rounded-radius-lg">
                                <div className="bg-primary/20 p-1.5 rounded-radius-md text-primary shrink-0">
            ...
            {/* BACKGROUND MAGIC SELECTION MODAL */}
            {showMagicModal && bgMagicConfig && createPortal(
                <div className="fixed inset-0 z-[110] bg-background-light dark:bg-background-dark flex flex-col pt-[max(0.75rem,env(safe-area-inset-top))] animate-fadeIn">
                    <div className="p-4 bg-white dark:bg-surface-dark border-b border-slate-200 dark:border-white/10 flex items-center justify-between shadow-md">
                        <button onClick={() => setShowMagicModal(false)} className="w-10 h-10 flex items-center justify-center rounded-radius-pill bg-slate-100 dark:bg-white/5 text-slate-500"><span className="material-symbols-outlined">close</span></button>
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-8">
                        ...
                        <section>
                            <div className="flex justify-between items-baseline mb-4">
                                ...
                                <span className={`text-xs font-black px-2 py-0.5 rounded-radius-pill ${bgSpells.filter(s => SPELL_DETAILS[s]?.level === 0).length === bgMagicConfig.cantripsNeeded ? 'bg-emerald-500/10 text-emerald-500' : 'bg-primary/10 text-primary'}`}>
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                {bgMagicConfig.sourceList.filter(s => SPELL_DETAILS[s]?.level === 0).map(name => {
                                    ...
                                    return (
                                        <button key={name} onClick={() => toggleBgSpell(name, 0)} className={`flex items-center gap-3 p-3 rounded-radius-xl border transition-all text-left ${isSel ? 'bg-primary/10 border-primary shadow-elev-modal scale-[1.02]' : 'bg-white dark:bg-surface-dark border-slate-200 dark:border-white/5 opacity-80 hover:opacity-100'}`}>
                                            <div className={`size-10 rounded-radius-lg flex items-center justify-center shrink-0 ${isSel ? 'bg-primary text-white shadow-inner' : theme.bg + ' ' + theme.text}`}>
                        ...
                        <section className="pb-10">
                            <div className="flex justify-between items-baseline mb-4">
                                ...
                                <span className={`text-xs font-black px-2 py-0.5 rounded-radius-pill ${bgSpells.filter(s => SPELL_DETAILS[s]?.level === 1).length === bgMagicConfig.level1Needed ? 'bg-emerald-500/10 text-emerald-500' : 'bg-primary/10 text-primary'}`}>
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                {bgMagicConfig.sourceList.filter(s => SPELL_DETAILS[s]?.level === 1).map(name => {
                                    ...
                                    const theme = SCHOOL_THEMES[spell.school] || { text: 'text-slate-400', bg: 'bg-slate-500/10', icon: 'auto_fix' };
                                    return (
                                        <button key={name} onClick={() => toggleBgSpell(name, 1)} className={`flex items-center gap-3 p-3 rounded-radius-xl border transition-all text-left ${isSel ? 'bg-primary/10 border-primary shadow-elev-modal scale-[1.02]' : 'bg-white dark:bg-surface-dark border-slate-200 dark:border-white/5 opacity-80 hover:opacity-100'}`}>
                                            <div className={`size-10 rounded-radius-lg flex items-center justify-center shrink-0 ${isSel ? 'bg-primary text-white shadow-inner' : theme.bg + ' ' + theme.text}`}>
                                                <span className="material-symbols-outlined text-lg">{isSel ? 'check' : theme.icon}</span>
                                            </div>
                                            <div className="min-w-0">
                    ...
                    <div className="p-4 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md border-t border-slate-200 dark:border-white/10 shadow-[0_-4px_20px_rgba(0,0,0,0.1)]">
                        <button 
                            ...
                            className={`w-full py-4 rounded-radius-xl font-black text-xs uppercase tracking-[0.2em] shadow-elev-modal transition-all active:scale-95 ${bgSpells.length === (bgMagicConfig.cantripsNeeded + bgMagicConfig.level1Needed) ? 'bg-primary text-background-dark shadow-primary/20' : 'bg-slate-200 dark:bg-white/5 text-slate-500 cursor-not-allowed'}`}
                        >
                            {t.confirm_selection}
                        </button>
                    </div>
            ...
            {/* URL INPUT MODAL */}
            {showUrlInput && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/80 animate-fadeIn backdrop-blur-sm">
                    <div className="w-[calc(100%-2rem)] sm:max-w-sm bg-white dark:bg-[#0f1525] rounded-radius-xl shadow-2xl animate-slideUp p-5 space-y-4"
                        onClick={(e) => e.stopPropagation()}>
                        <h3 className="text-sm font-black text-slate-900 dark:text-white">Paste Image URL</h3>
                        <input autoFocus value={urlInput} onChange={e => setUrlInput(e.target.value)}
                            className="w-full bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-radius-lg px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500/50" />
                        <div className="flex gap-2">
                            <button onClick={() => setShowUrlInput(false)}
                                className="flex-1 py-2.5 rounded-radius-lg font-bold text-sm bg-slate-100 dark:bg-white/10 text-slate-700 dark:text-white hover:bg-slate-200 dark:hover:bg-white/20">Cancel</button>
                            <button onClick={() => {
                                if (urlInput.length > 5000) {
                                    void dialog.showAlert("Image URL is too long. Please use a shorter direct link.");
                                } else {
                                    setCharImage(urlInput || DEFAULT_CHAR_IMAGE);
                                    setShowUrlInput(false);
                                    setShowImageOptions(false);
                                }
                            }} className="flex-1 py-2.5 rounded-radius-lg font-bold text-sm text-white bg-blue-600 hover:bg-blue-700">Apply</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {showSkilledModal && bgSkilledConfig && createPortal(
                <div className="fixed inset-0 z-[110] bg-background-light dark:bg-background-dark flex flex-col pt-[max(0.75rem,env(safe-area-inset-top))] animate-fadeIn">
                    <div className="p-4 bg-white dark:bg-surface-dark border-b border-slate-200 dark:border-white/10 flex items-center justify-between shadow-md">
                        <button onClick={() => setShowSkilledModal(false)} className="w-10 h-10 flex items-center justify-center rounded-radius-pill bg-slate-100 dark:bg-white/5 text-slate-500"><span className="material-symbols-outlined">close</span></button>
                        <div className="text-center">
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-4">
                        <div className="flex justify-between items-baseline mb-4">
                            ...
                            <span className={`text-xs font-black px-2 py-0.5 rounded-radius-pill ${bgSkilledSkills.length === bgSkilledConfig.skillsNeeded ? 'bg-emerald-500/10 text-emerald-500' : 'bg-primary/10 text-primary'}`}>
                                {bgSkilledSkills.length} / {bgSkilledConfig.skillsNeeded}
                            </span>
                        </div>
                        <div className="grid grid-cols-1 gap-2">
                            {bgSkilledConfig.sourceList.map(skillName => {
                                ...
                                return (
                                    <button key={skillName} onClick={() => toggleBgSkill(skillName)} className={`flex items-center gap-3 p-3 rounded-radius-xl border transition-all text-left ${isSel ? 'bg-primary/10 border-primary shadow-elev-modal scale-[1.02]' : 'bg-white dark:bg-surface-dark border-slate-200 dark:border-white/5 opacity-80 hover:opacity-100'}`}>
                                        <div className={`size-10 rounded-radius-lg flex items-center justify-center shrink-0 ${isSel ? 'bg-primary text-white shadow-inner' : 'bg-slate-100 dark:bg-white/5 text-slate-500'}`}>
                    ...
                    <div className="p-4 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md border-t border-slate-200 dark:border-white/10 shadow-[0_-4px_20px_rgba(0,0,0,0.1)]">
                        <button 
                            ...
                            className={`w-full py-4 rounded-radius-xl font-black text-xs uppercase tracking-[0.2em] shadow-elev-modal transition-all active:scale-95 ${bgSkilledSkills.length === bgSkilledConfig.skillsNeeded ? 'bg-primary text-background-dark shadow-primary/20' : 'bg-slate-200 dark:bg-white/5 text-slate-500 cursor-not-allowed'}`}
                        >
                            {t.confirm_selection}
                        </button>
                    </div>
