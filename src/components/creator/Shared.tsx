...
export const SectionSeparator = ({ label }: { label: string }) => (
  <div className="relative py-6 flex items-center justify-center w-full px-6">
    ...
    <div className="relative flex justify-center">
      <span className="bg-background-light dark:bg-background-dark px-4 text-xs font-bold text-slate-400 uppercase tracking-widest border border-slate-200 dark:border-white/10 rounded-radius-pill py-1 shadow-sm">
        {label}
      </span>
    </div>
  </div>
...
export const ClassProgressionList = ({ selectedClass, subclassData, currentLevel }: { selectedClass: string, subclassData?: SubclassData, currentLevel: number }) => {
  ...
  return (
    <div className="mt-6 space-y-3 px-6">
      <div className="border rounded-radius-xl overflow-hidden transition-all duration-motion-base shadow-sm" style={{borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)'}}>
        <button
        ...
        >
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-radius-lg flex items-center justify-center transition-colors ${isOpen ? 'bg-primary text-white' : 'bg-primary/10 text-primary'}`}>
          ...
          <span className={`material-symbols-outlined text-slate-400 transition-transform duration-motion-base ${isOpen ? 'rotate-180 text-primary' : ''}`}>
            expand_more
          </span>
        </button>

        <div className={`transition-[grid-template-rows] duration-motion-slow ease-in-out grid ${isOpen ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
              ...
              {Array.from({ length: 20 }, (_, i) => i + 1).map((lvl) => {
                ...
                return (
                  <div key={lvl} className="relative pl-6 pb-6 border-l-2 border-slate-200 dark:border-slate-700 ml-2 last:border-0 last:pb-0">
                    <div className={`absolute -left-[9px] top-0 w-4 h-4 rounded-radius-pill border-2 transition-colors ${lvl <= currentLevel ? 'bg-primary border-primary' : 'bg-background-light dark:bg-background-dark border-slate-300 dark:border-slate-600'}`}></div>
                    ...
                    <div className="flex flex-col gap-1 -mt-1.5">
                      ...
                      <div className="space-y-2 mt-1">
                        {features.map((feat, idx) => (
                          <div key={idx} className={`bg-white dark:bg-surface-dark p-3 rounded-radius-md border shadow-sm ${lvl <= currentLevel ? 'border-primary/30' : 'border-slate-200 dark:border-white/5'}`}>
