    ...
    return (
        <div className="px-6 py-4 space-y-6">
             <div className="flex flex-col items-center">
                <div className="w-24 h-24 rounded-radius-2xl bg-slate-200 dark:bg-surface-light mb-3 shadow-elev-modal border-2 border-white" style={{backgroundImage: `url(${charImage})`, backgroundSize: 'cover', backgroundPosition: 'center'}}></div>
                ...
                <div className="flex flex-wrap justify-center items-center gap-2 mt-1">
                    ...
                    <span className="w-1 h-1 rounded-radius-pill bg-slate-400"></span>
                    <span className="text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wide">{selectedClass}</span>
                    {selectedSubclass && <><span className="w-1 h-1 rounded-radius-pill bg-slate-400"></span><span className="text-sm font-bold text-slate-500 max-w-[150px] truncate">{selectedSubclass}</span></>}
                    <span className="w-1 h-1 rounded-radius-pill bg-slate-400"></span>
             ...
             <div className="grid grid-cols-4 gap-3">
                {[
                  ...
                  { l: 'INI', v: init, i: 'bolt', c: 'text-yellow-500' }  // Use t.initiative if short enough
                    ...
                    <div key={s.l} className="flex flex-col items-center p-2.5 rounded-radius-xl bg-white dark:bg-surface-dark border border-slate-200 shadow-sm">
             ...
             <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                    {Object.entries(finalStats).map(([key, val]) => (
                        <div key={key} className="flex justify-between items-center p-2 rounded-radius-lg bg-slate-100 dark:bg-white/5 border border-transparent">
             ...
             <div className="bg-white dark:bg-surface-dark rounded-radius-xl p-4 border border-slate-200 space-y-4 shadow-sm">
             ...
             <div>
                ...
                <div className="flex flex-wrap gap-2">{trainedSkills.map(skill => <div key={skill} className="flex items-center gap-1.5 px-3 py-1.5 rounded-radius-md bg-primary/10 border border-primary/20 text-primary"><span className="material-symbols-outlined text-[14px]">check</span><span className="text-xs font-bold">{skill}</span></div>)}</div>
             </div>

             <button onClick={onConfirm} className="w-full py-4 bg-primary text-background-dark font-bold rounded-radius-lg shadow-[0_8px_30px_rgb(53,158,255,0.4)] text-lg hover:scale-[1.02] transition-transform mt-4">{t.confirm_character}</button>
        </div>
    );
};
