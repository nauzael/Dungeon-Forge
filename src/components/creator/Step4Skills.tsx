...
const Step4Skills: React.FC<Step4Props> = ({
    ...
    return (
        <div className="px-6 py-4 space-y-6">
            {selectedSpecies === 'Elf' && (
                <div className="bg-green-50 dark:bg-green-900/10 p-4 rounded-radius-lg border border-green-100 mb-4 animate-fadeIn">
                    ...
                    <select value={selectedElfSkill} onChange={(e) => setSelectedElfSkill(e.target.value)} className="w-full bg-white dark:bg-surface-dark border border-slate-200 rounded-radius-lg px-4 py-3 text-sm font-bold">
            ...
            {selectedSpecies === 'Human' && (
                <div className="bg-blue-50 dark:bg-blue-900/10 p-4 rounded-radius-lg border border-blue-100">
                    ...
                    <select value={selectedHumanSkill} onChange={(e) => setSelectedHumanSkill(e.target.value)} className="w-full bg-white dark:bg-surface-dark border border-slate-200 rounded-radius-lg px-4 py-3 text-sm font-bold">
            ...
            <div>
                ...
                <div className="grid grid-cols-1 gap-2">
                    {SKILL_LIST.map(skill => {
                        ...
                        return (
                            <button key={skill} onClick={() => !isBackground && !isHumanSelected && !isElfSelected && !isBgSkilled && toggleSkill(skill)} disabled={isBackground || isHumanSelected || isElfSelected || isBgSkilled} className={`flex items-center justify-between p-3.5 rounded-radius-lg border transition-all ${isBackground || isHumanSelected || isElfSelected || isBgSkilled ? 'bg-slate-100 dark:bg-white/5 border-transparent opacity-80' : isSelected ? 'bg-primary/5 border-primary shadow-[0_0_15px_rgba(53,158,255,0.2)]' : 'bg-white dark:bg-surface-dark border-slate-200 hover:border-primary/50'}`}>
                                ...
                                <div className="flex items-center gap-3"><span className={`text-xs font-bold ${isProf ? 'text-primary' : 'text-slate-400'}`}>{total >= 0 ? '+' : ''}{total}</span><div className={`w-5 h-5 rounded-radius-pill border flex items-center justify-center ${isProf ? 'bg-primary border-primary' : 'border-slate-300 dark:border-slate-600'}`}>{isProf && <span className="material-symbols-outlined text-sm text-black font-bold">check</span>}</div></div>
                            </button>
                        );
                    })}
                </div>
                ...
                <div className="mt-8 animate-fadeIn">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                        {metamagicOptions.map(m => (
                            <button key={m.name} onClick={() => toggleMetamagic(m.name)} disabled={!selectedMetamagics.includes(m.name) && selectedMetamagics.length >= maxMetamagics} className={`p-4 rounded-radius-lg border text-left transition-all ${selectedMetamagics.includes(m.name) ? 'bg-primary/5 border-primary shadow-[0_0_15px_rgba(53,158,255,0.2)]' : 'bg-white dark:bg-surface-dark border-slate-200 hover:border-primary/50'}`}>
                            ...
                            return (
                                <div key={idx} className="space-y-1.5 animate-slideUp" style={{ animationDelay: `${idx * 0.1}s` }}>
                                    ...
                                    <button 
                                        ...
                                        className={`w-full text-left p-4 rounded-radius-xl border transition-all flex flex-col gap-2 ${
                                            selectedWeapon 
                                        ...
                                        {!selectedWeapon ? (
                                            <div className="flex items-center justify-between opacity-60 group">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-10 h-10 rounded-radius-lg bg-slate-100 dark:bg-white/5 flex items-center justify-center">
                                                ...
                                                <div className="flex justify-between items-start">
                                                    <div>
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-radius-sm bg-primary text-background-dark uppercase">
                                                    ...
                                                    <span className="material-symbols-outlined text-primary bg-primary/10 p-1.5 rounded-radius-md">edit</span>
                                                </div>
                                                <div className="p-3 bg-white/50 dark:bg-black/20 rounded-radius-lg border border-primary/10">
                ...
                <div className="mt-8 animate-fadeIn pb-10">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                        {SKILL_LIST.map(skill => {
                            ...
                            return (
                                <button key={skill} onClick={() => toggleExpertise(skill)} disabled={isDisabled} className={`flex items-center justify-between p-4 rounded-radius-lg border transition-all ${isSelected ? 'bg-primary/5 border-primary shadow-[0_0_15px_rgba(53,158,255,0.2)]' : isDisabled ? 'opacity-40' : 'bg-white dark:bg-surface-dark border-slate-200'}`}>
