  ...
  return (
    <header className="sticky top-0 z-50 bg-[#0f172a]/80 backdrop-blur-xl border-b border-white/5 px-6 pt-[max(1.5rem,env(safe-area-inset-top))] pb-4 flex items-center justify-between">
          ...
          {isEditingName ? (
            <div className="flex items-center gap-2">
              <input 
                ...
                className="bg-white/5 border border-white/20 rounded-radius-sm px-2 py-0.5 text-base font-bold text-white focus:outline-none focus:border-blue-500"
              />
            </div>
          ) : (
              ...
              <>
                ...
                <div 
                  onClick={async () => {
                    ...
                    await dialog.showAlert("Share code copied!");
                  }}
      ...
      <div className="flex items-center gap-2">
        {party && (
          <>
            <div className={`flex items-center gap-1.5 px-2 py-1 rounded-radius-pill text-[8px] font-black uppercase tracking-tighter border ${realtimeStatus === 'connected' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-amber-500/10 text-amber-500 border-amber-500/20 animate-pulse'}`}>
            ...
            <button 
              onClick={onSync}
              className="w-10 h-10 flex items-center justify-center rounded-radius-pill bg-white/5 border border-white/10 text-slate-400 hover:text-white transition-colors"
              title="Sync Manually"
            >
            ...
            <button 
              onClick={onDelete}
              className="w-10 h-10 flex items-center justify-center rounded-radius-pill bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500 hover:text-white transition-all shadow-md shadow-red-900/10"
              title="Delete Table"
            >
        ...
        <button onClick={onBack} className="w-10 h-10 flex items-center justify-center rounded-radius-pill bg-white/5 border border-white/10 text-slate-400 hover:text-white transition-colors">
