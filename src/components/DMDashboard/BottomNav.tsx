...
const BottomNav: React.FC<BottomNavProps> = ({ activeTab, onTabChange }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[100] px-4 pb-6 pt-4 bg-gradient-to-t from-[#0f172a] via-[#0f172a] to-transparent">
      <nav className="mx-auto max-w-sm bg-slate-900 border border-white/10 rounded-radius-2xl p-2 grid grid-cols-6 gap-1 shadow-2xl backdrop-blur-xl">
        {tabs.map(tab => (
          <button 
            key={tab.id}
            onClick={() => onTabChange(tab.id)}
            className={`flex flex-col items-center gap-1 py-1.5 rounded-radius-xl transition-all ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-elev-modal shadow-blue-500/20' : 'text-slate-500 hover:text-slate-300'}`}
          >
