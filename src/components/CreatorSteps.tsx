...
const CreatorSteps: React.FC<CreatorStepsProps> = ({ onBack, onFinish }) => {
  ...
  const [baseStats, setBaseStats] = useState<Record<Ability, number>>({
    STR: 8,
    DEX: 8,
    CON: 8,
    INT: 8,
  ...
  const [featModalContext, setFeatModalContext] = useState<{
    type: 'human' | 'asi';
    level?: number;
  } | null>(null);
  ...
  const asiLevels = useMemo(() => {
    ...
    for (let l = 1; l <= level; l++) {
      if (
        progression[l]?.some(
          (f) => f.includes('Ability Score Improvement') || f.includes('Epic Boon')
        )
      )
        levels.push(l);
    }
  ...
  const finalStats = useMemo(() => {
    ...
    if (backgroundData?.scores) {
      if (bgAsiMode === 'three')
        backgroundData.scores.forEach(
          (ability) => (stats[ability] = Math.min(20, stats[ability] + 1))
        );
      else {
    ...
    asiLevels.forEach((lvl) => {
      ...
      } else if (decision?.type === 'feat' && decision.stat1)
        stats[decision.stat1] = Math.min(20, stats[decision.stat1] + 1);
    });
    return stats;
  ...
  const usedPoints = useMemo(
    () =>
      Object.values(baseStats).reduce(
        (acc: number, val) => acc + (POINT_BUY_COSTS[val as number] || 0),
        0
      ),
    [baseStats]
  );
  ...
  const toggleSkill = (skill: string) => {
    ...
    else if (selectedSkills.length < classSkillOptions.count)
  ...
  const toggleMetamagic = (meta: string) => {
    ...
    else if (selectedMetamagics.length < maxMetamagics)
  ...
  const toggleExpertise = (skill: string) => {
    ...
    else if (selectedExpertise.length < maxExpertise)
  ...
  const calculateAC = () => {
    ...
    else if (selectedClass === 'Pugilist') ac = 12 + conMod;
    ...
    const allFeats = [
      ...
      selectedFeat,
      ...asiLevels.map((l) =>
        asiDecisions[l]?.type === 'feat' ? asiDecisions[l]?.feat : undefined
      ),
    ].filter((f): f is string => !!f);

    // Bonus de Dotes
    if (allFeats.some((f) => f.includes('Dual Wielder'))) ac += 1;
    if (allFeats.some((f) => f.includes('Defense'))) ac += 1; // Fighting Style

    return ac;
  };

  const calculateMaxHP = () => {
    const hasAmuletOfHealth = (backgroundData?.equipment || []).some(
      (e) =>
        e.toLowerCase().includes('amulet of health') || e.toLowerCase().includes('amuleto de salud')
    );
  ...
  const renderHpBreakdown = () => {
    ...
    const conMod = Math.floor(
      ((hasAmuletOfHealth ? Math.max(finalStats.CON || 10, 19) : finalStats.CON || 10) - 10) / 2
    );
    const hitDie = HIT_DIE[selectedClass] || 8;
    ...
    if (selectedClass === 'Sorcerer' && selectedSubclass === 'Draconic Sorcery')
      bonuses.push({ name: t.draconicResilience, val: level });
    const allFeats = [
      backgroundData?.feat,
      selectedFeat,
      ...asiLevels.map((l) =>
    ...
    return (
      <div className="mt-3 p-4 rounded-radius-lg shadow-inner" 
        style={{
        ...
        {level > 1 && (
          <div className="flex justify-between border-b pt-1" style={{ borderColor: 'var(--color-border)' }}>
            ...
            <span>
              {hpMethod === 'average'
                ? (avgRoll + conMod) * (level - 1)
                : manualRolledHP + conMod * (level - 1)}
            </span>
          </div>
        )}
        {bonuses.map((b, i) => (
          <div
            key={i}
            className="flex justify-between text-primary border-t border-slate-200 pt-1 border-dashed"
          >
            <span>{b.name}:</span>
            <span>+{b.val}</span>
          </div>
        ))}
        <div className="border-t-2 border-slate-200 mt-2 pt-2 flex justify-between text-sm font-bold text-slate-900 dark:text-white">
          <span>{t.total}:</span>
  ...
  const calculateSpeed = () => {
    ...
    if (selectedClass === 'Monk' && level >= 2)
      speed += level >= 18 ? 30 : level >= 14 ? 25 : level >= 10 ? 20 : level >= 6 ? 15 : 10;
    ...
    if (allFeats.some((f) => f.includes('Boon of Speed') || f.includes('Bono de Velocidad')))
      speed += 30;
  ...
  const nextStep = () => {
    if (step < 5) setStep((s) => (s + 1) as CreatorStep);
      ...
      const newCharacter: Character = {
        ...
        focus: selectedClass === 'Monk' || selectedClass === 'Pugilist' ? { current: level, max: level } : undefined,
        lucky: undefined,
        ...
        featSpells: [...bgSpells], // Magic Initiate spells are free, don't count against class limits
        innateSpells: speciesInnateSpells.innateSpells,
        inventory: useStartingGold
          ? []
          : [trinket, ...(backgroundData?.equipment || [])].map((itemStr, i) => {
              ...
              return {
                ...
                equipped:
                  baseName.toLowerCase().includes('amuleto') ||
                  baseName.toLowerCase().includes('amulet'),
              };
            }),
      ...
      if (selectedClass === 'Pugilist') {
        newCharacter.kiMax = level; // Moxie Points (reuses kiMax as generic resource pool)
      }
      if (selectedClass === 'Rogue') {
      ...
      if (selectedClass === 'Druid') {
        ...
        newCharacter.wildShapeForms = ['Wolf', 'Rat', 'Spider', 'Riding Horse'];
      }
      if (selectedClass === 'Ranger') {
        newCharacter.hunterMarkUses = { current: 2, max: 2 };
      }
      if (selectedClass === 'Warlock') {
        newCharacter.pactSlotLevel = 1;
        newCharacter.magicalCunning = { current: 1, max: 1 };
        newCharacter.mysticArcanum = {};
      }
      if (selectedClass === 'Wizard') {
        newCharacter.arcaneRecovery = { uses: 1 };
      }
    ...
    <div className="flex flex-col h-full min-h-screen relative bg-background-light dark:bg-background-dark">
      <header className="flex items-center justify-between p-4 pt-[calc(1.5rem+env(safe-area-inset-top))] z-10 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-sm sticky top-0">
        <button
          ...
          className="w-10 h-10 flex items-center justify-center rounded-radius-pill hover:bg-black/5 transition-colors"
        >
          <span className="material-symbols-outlined">arrow_back</span>
        </button>
        <div className="flex-1 text-center">
          <h2 className="text-lg font-bold">
            {step === 5 ? t.resume : `${t.step} ${step} ${t.of} 5`}
          </h2>
        </div>
      ...
      {step < 5 && (
        <div className="px-6 pb-2 flex gap-2">
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className={`h-1.5 flex-1 rounded-radius-pill transition-all ${
                i <= step
      ...
      <main ref={mainRef} className="flex-1 overflow-y-auto no-scrollbar pb-5">
        {step === 1 && (
          <Step1Identity
            ...
            setSelectedSubspecies={setSelectedSubspecies}
            selectedBackground={selectedBackground}
            setSelectedBackground={setSelectedBackground}
            selectedFeat={selectedFeat}
            bgSpells={bgSpells}
            setBgSpells={setBgSpells}
            bgSkilledSkills={bgSkilledSkills}
            setBgSkilledSkills={setBgSkilledSkills}
          />
        )}
        {step === 2 && (
          <Step2Stats
            ...
            bgAsiMode={bgAsiMode}
            setBgAsiMode={setBgAsiMode}
            bgPlus2={bgPlus2}
            setBgPlus2={setBgPlus2}
            bgPlus1={bgPlus1}
            setBgPlus1={setBgPlus1}
            ...
            manualRolledHP={manualRolledHP}
            setManualRolledHP={setManualRolledHP}
            level={level}
            renderHpBreakdown={renderHpBreakdown}
            asiLevels={asiLevels}
            asiDecisions={asiDecisions}
            handleAsiChange={handleAsiChange}
            openFeatModal={(ctx) => {
              setFeatModalContext(ctx);
              setShowFeatModal(true);
            }}
            suggestedArray={CLASS_SUGGESTED_ARRAYS[selectedClass]}
        ...
        {step === 3 && (
          <Step3Details
            ...
            useStartingGold={useStartingGold}
            setUseStartingGold={setUseStartingGold}
          />
        )}
        {step === 4 && (
          <Step4Skills
            selectedSpecies={selectedSpecies}
            selectedElfSkill={selectedElfSkill}
            setSelectedElfSkill={setSelectedElfSkill}
            selectedHumanSkill={selectedHumanSkill}
            setSelectedHumanSkill={setSelectedHumanSkill}
            ...
            toggleMetamagic={toggleMetamagic}
            maxMasteries={maxMasteries}
            selectedMasteries={selectedMasteries}
            setMasteryAtIndex={setMasteryAtIndex}
      ...
      {step < 5 && (
        <div className="px-4 py-3 sticky bottom-0 z-20 mb-[env(safe-area-inset-bottom)]" 
          ...
          <button
            onClick={nextStep}
            className="w-full py-3 rounded-radius-lg font-bold text-lg bg-primary text-background-dark shadow-elev-modal shadow-primary/30"
          >
            {t.continue}
          </button>
        </div>
      ...
      {showFeatModal && (
        <FeatModal
          featModalContext={featModalContext}
          selectedFeat={selectedFeat}
          asiDecisions={asiDecisions}
          handleFeatSelect={handleFeatSelect}
