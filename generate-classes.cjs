...
const MANUAL = fs.readFileSync('docs/Manual/01-classes.md', 'utf8');
const OUTPUT = 'Data/classes/classes-en.ts';

let out = `import { Ability, Skill, DetailData, SubclassData } from '../../types';
...
function getCoreTraits(section) {
  const traits = [];
  const re = /^\s*-\s+\*\*([^*]+)\*\*:\s*(.+)$/gm;
  let m;
...
function extractClassData(md, className) {
  const section = getSection(md, className);
  if (!section) return null;

  const data = { name: className, subclasses: [], progression: {} };
  ...
  // Core Traits
  const traits = getCoreTraits(section);
  traits.forEach(t => {
    if (t.name === 'Primary Ability') data.primaryAbility = t.desc;
    if (t.name === 'Hit Point Die') data.hitDie = parseInt(t.desc.match(/D(\d+)/)[1]);
    if (t.name === 'Saving Throw Proficiencies') {
      data.savingThrows = t.desc.split(/,\s*|\band\b/i).map(s => s.trim().toUpperCase().substring(0, 3));
    }
    if (t.name === 'Skill Proficiencies') {
      const countMatch = t.desc.match(/Choose\s*(\d+)/i);
      const optsMatch = t.desc.match(/Choose\s*\d+:\s*(.+)/i);
      if (optsMatch) {
        data.skills = optsMatch[1].split(',').map(s => s.trim().replace(/\s*or\s.+$/, '').trim());
      }
    }
  });

  // Subclasses
  const subMatch = section.match(/\*\*Subclases?:\*\*\s*\n((?:-\s+.+\n)+)/);
  if (subMatch) {
    const subLines = subMatch[1].split('\n').filter(l => l.trim());
    subLines.forEach(line => {
      const name = line.replace(/^-\s*/, '').trim();
      if (name) data.subclasses.push(name);
    });
  }
...
classNames.forEach(name => {
  const d = extractClassData(MANUAL, name);
  if (d) {
    console.log(`${name}: HD=${d.hitDie}, ST=[${d.savingThrows.join(',')}], Subs=${d.subclasses.length}`);
  }
});

console.log('\nParsing complete. Writing to output...');

// Now write the full classes-en.ts with complete data
// Using the actual manual data to create the full file
...
const classData = {
  Artificer: {
    ...
    skillData: { count: 2, options: ['Arcana', 'History', 'Investigation', 'Medicine', 'Nature', 'Perception', 'Sleight of Hand'] },
    masteriesCount: 2,
    ...
    subclasses: [
      {
        ...
        features: {
          ...
          5: [{ name: 'Alchemical Savant', description: 'When casting spell with Alchemist\'s Supplies as Focus, add Int modifier to one roll that restores HP or deals Acid/Fire/Poison damage.' }],
          9: [{ name: 'Restorative Reagents', description: 'Cast Lesser Restoration without slot, without preparing, using Alchemist\'s Supplies. Uses equal to Int modifier per Long Rest.' }],
          15: [{ name: 'Chemical Mastery', description: 'Alchemical Eruption: when Artificer spell deals Acid/Fire/Poison damage, add 2d8 Force damage. Chemical Resistance: Resistance to Acid and Poison, immunity to Poisoned. Conjured Cauldron: cast Tasha\'s Bubbling Cauldron without slot/components using Alchemist\'s Supplies. Once per Long Rest.' }]
        }
      },
      {
        name: 'Armorer',
        ...
        features: {
          ...
          5: [{ name: 'Extra Attack', description: 'Attack twice instead of once.' }],
          9: [{ name: 'Improved Armorer', description: 'Learn additional Replicate Magic Item plan (Armor category). Can create two items with that feature. Improved Arsenal: +1 to attack/damage with armor weapon.' }],
          15: [{ name: 'Perfected Armor', description: 'Dreadnaught: demolisher damage 2d6; Giant Stature reach +10, can become Huge, Advantage on Strength checks/saves. Guardian: Thunder Pulse damage 1d10; Reaction pull Huge or smaller creature 25 feet toward you, attack if within 5 feet. Infiltrator: Lightning Launcher 2d6; target glimmers (Disadvantage on attacks against you); Bonus Action Fly Speed twice Speed.' }]
        }
      },
      {
        name: 'Artillerist',
        ...
        features: {
          ...
          5: [{ name: 'Arcane Firearm', description: 'Long Rest carve sigils into Rod/Staff/Wand/Martial Ranged weapon to make Arcane Firearm. Use as Spellcasting Focus. When casting Artificer spell through it, roll 1d8 and add to one damage roll.' }],
          9: [{ name: 'Explosive Cannon', description: 'Detonate: when cannon takes damage, Reaction detonate (destroy cannon, 3d10 Force to creatures within 20 feet, Dex save). Firepower: cannon damage and Protector Temporary Hit Points increase by 1d8.' }],
          15: [{ name: 'Fortified Position', description: 'Double Firepower: two cannons at once, create both with same action. Shimmering Field: Half Cover for you and allies within 10 feet of cannon.' }]
        }
      },
      {
        name: 'Battle Smith',
        ...
        features: {
          ...
          5: [{ name: 'Extra Attack', description: 'Attack twice instead of once.' }],
          9: [{ name: 'Arcane Jolt', description: 'When you hit with weapon attack, deal extra 2d6 Force damage. When dealing damage, choose ally within 30 feet to heal 2d6 HP. Uses equal to Int modifier per Long Rest.' }],
          15: [{ name: 'Improved Steel Defender', description: 'Steel Defender gains: Immune to your spells; Bonus to AC equal to Int modifier; attack uses d8; Defensive/Defiant improved; can take Dash, Disengage, Help, or Use an Object as Bonus Action.' }]
        }
      },
      {
        name: 'Cartographer',
        ...
        features: {
          ...
          5: [{ name: 'Expanding Horizons', description: 'Strategic Mark range increases to 1 mile. When casting Teleportation Circle, create temporary circle (lasts 1 hour) that doesn\'t require materials.' }],
          9: [{ name: "Cartographer's Eye", description: 'As Bonus Action, cast Locate Object at will (range 300 feet). When casting spells from Cartographer Spells, add Proficiency Bonus to Survival checks to avoid getting lost.' }],
          15: [{ name: 'Perfect Cartography', description: 'Strategic Mark marks an area (100-foot Cube). When you enter marked area, choose: gain Tremorsense 60 feet for 1 minute; or create three magic mouth alarms that last until rest. When casting Teleportation Circle, it becomes permanent without requiring a key object.' }]
        }
      }
    ]
  },
  Barbarian: {
    ...
    skillData: { count: 2, options: ['Animal Handling', 'Athletics', 'Intimidation', 'Nature', 'Perception', 'Survival'] },
    masteriesCount: 2,
    ...
    subclasses: [
      ...
      },
      {
        ...
        features: {
          ...
          6: [{ name: 'Aspect of the Wilds', description: 'You gain one of the following options of your choice: Owl (Darkvision with a range of 60 feet); Panther (Climb Speed equal to your Speed); Salmon (Swim Speed equal to your Speed).' }],
          10: [{ name: 'Nature Speaker', description: 'You can cast the Commune with Nature spell but only as a Ritual.' }],
      ...
      },
      {
        ...
        features: {
          ...
          10: [{ name: 'Battering Roots', description: 'During your turn, your reach is 10 feet greater with any Melee weapon that has the Heavy or Versatile property.' }],
          14: [{ name: 'Travel along the Tree', description: 'When you activate your Rage and as a Bonus Action while your Rage is active, you can teleport up to 60 feet to an unoccupied space you can see.' }]
        }
      },
      {
        ...
        features: {
          ...
          6: [{ name: 'Fanatical Focus', description: 'Once per active Rage, if you fail a saving throw, you can reroll it with a bonus equal to your Rage Damage bonus.' }],
          10: [{ name: 'Zealous Presence', description: 'As a Bonus Action, you unleash a battle cry. Up to ten other creatures of your choice within 60 feet of you gain Advantage on attack rolls and saving throws until the start of your next turn. Once you use this feature, you can\'t use it again until you finish a Long Rest unless you expend a use of your Rage.' }],
          14: [{ name: 'Rage of the Gods', description: 'When you activate your Rage, you can assume the form of a divine warrior for 1 minute or until you drop to 0 Hit Points. While in this form, you gain Flight, Resistance to Necrotic, Psychic, and Radiant damage, and when a creature within 30 feet of you would drop to 0 Hit Points, you can expend a use of your Rage to instead change the target\'s Hit Points to a number equal to your Barbarian level.' }]
        }
      }
    ]
  },
  Bard: {
    ...
    skillData: { count: 3, options: ['Any 3 skills'] },
    masteriesCount: 0,
    ...
    subclasses: [
      {
        ...
        features: {
          ...
          14: [{ name: 'Leading Evasion', description: 'When you are subjected to an effect that allows you to make a Dexterity saving throw to take only half damage, you instead take no damage if you succeed and only half if you fail. If any creatures within 5 feet of you are making the same save, you can share this benefit.' }]
        }
      },
      {
        ...
        features: {
          ...
          6: [{ name: 'Mantle of Majesty', description: 'You always have the Command spell prepared. As a Bonus Action, you cast Command without expending a spell slot, and you take on an unearthly appearance for 1 minute. Any creature Charmed by you automatically fails its saving throw against Command.' }],
          14: [{ name: 'Unbreakable Majesty', description: 'As a Bonus Action, you can assume a magically majestic presence for 1 minute or until you have the Incapacitated condition. Whenever any creature hits you with an attack roll for the first time on a turn, the attacker must succeed on a Charisma saving throw or the attack misses.' }]
        }
      },
      {
        ...
        features: {
          ...
          6: [{ name: 'Magical Discoveries', description: 'You learn two spells of your choice from the Cleric, Druid, or Wizard spell list. A spell you choose must be a cantrip or a spell for which you have spell slots.' }],
          14: [{ name: 'Peerless Skill', description: 'When you make an ability check or attack roll and fail, you can expend one use of Bardic Inspiration and add the number rolled to the d20, potentially turning a failure into a success.' }]
        }
      },
      {
      ...
      },
      {
        ...
        features: {
          ...
          6: [{ name: 'Extra Attack', description: 'You can attack twice instead of once whenever you take the Attack action on your turn. You can cast one of your cantrips in place of one of those attacks.' }],
  ...
  Cleric: {
    ...
    statPriorities: ['WIS', 'STR', 'CON'],
    skillData: { count: 2, options: ['History', 'Insight', 'Medicine', 'Persuasion', 'Religion'] },
    masteriesCount: 0,
    ...
    subclasses: [
      {
        ...
        features: {
          ...
          6: [{ name: 'Unfettered Mind', description: 'You gain telepathy out to 60 feet. When you use this telepathy, you can simultaneously contact a number of creatures equal to your Wisdom modifier. Additionally, you gain proficiency in Intelligence saving throws.' }],
          17: [{ name: 'Divine Foreknowledge', description: 'As a Bonus Action, you magically expand your mind to the future. For 1 hour, you have Advantage on D20 Tests. Once you use this feature, you can\'t use it again until you finish a Long Rest.' }]
        }
      },
      {
        name: 'Life Domain',
        ...
        features: {
          ...
          17: [{ name: 'Supreme Healing', description: 'When you would normally roll one or more dice to restore Hit Points to a creature with a spell or Channel Divinity, use the highest number possible for each die instead.' }]
        }
      },
      {
        name: 'Light Domain',
        ...
        features: {
          ...
          6: [{ name: 'Improved Warding Flare', description: 'Whenever you use Warding Flare, you can give the target of the triggering attack Temporary Hit Points equal to 2d6 plus your Wisdom modifier.' }],
          17: [{ name: 'Corona of Light', description: 'As a Magic action, you cause yourself to emit an aura of sunlight that lasts for 1 minute. You emit Bright Light in a 60-foot radius. Your enemies in the Bright Light have Disadvantage on saving throws against your Radiance of the Dawn and any spell that deals Fire or Radiant damage.' }]
        }
      },
      {
        ...
        features: {
          ...
          17: [{ name: 'Improved Duplicity', description: 'The illusion of your Invoke Duplicity has grown more powerful. When you and your allies make attack rolls against a creature within 5 feet of the illusion, the attack rolls have Advantage. When the illusion ends, you or a creature of your choice within 5 feet of it regains Hit Points equal to your Cleric level.' }]
        }
      },
      {
        name: 'War Domain',
        ...
        features: {
          ...
          6: [{ name: "War God's Blessing", description: 'You can expend a use of your Channel Divinity to cast Shield of Faith or Spiritual Weapon rather than expending a spell slot. When you cast either spell in this way, the spell doesn\'t require Concentration.' }],
  ...
  Druid: {
    ...
    statPriorities: ['WIS', 'CON', 'DEX'],
    skillData: { count: 2, options: ['Arcana', 'Animal Handling', 'Insight', 'Medicine', 'Nature', 'Perception', 'Religion', 'Survival'] },
    masteriesCount: 0,
    ...
    subclasses: [
      {
        ...
        features: {
          3: [{ name: "Land's Aid", description: 'As a Magic action, you can expend a use of your Wild Shape and choose a point within 60 feet. Vitality-giving flowers and life-draining thorns appear in a 10-foot-radius Sphere. Each creature of your choice must make a Constitution saving throw, taking 2d6 Necrotic damage on a failed save. One creature of your choice in that area regains 2d6 Hit Points. The damage and healing increase by 1d6 at levels 10 and 14.' }],
          6: [{ name: 'Natural Recovery', description: 'You can cast one of your level 1+ Circle Spells without expending a spell slot, and you must finish a Long Rest before you do so again. When you finish a Short Rest, you can choose expended spell slots to recover.' }],
          ...
          14: [{ name: "Nature's Sanctuary", description: 'As a Magic action, you can expend a use of your Wild Shape and cause spectral trees and vines to appear in a 15-foot Cube within 120 feet. You and your allies have Half Cover while in that area, and your allies gain the current Resistance of your Nature\'s Ward.' }]
        }
      },
      {
        ...
        features: {
          ...
          6: [{ name: 'Improved Circle Forms', description: 'Lunar Radiance (Each of your attacks in Wild Shape can deal its normal damage type or Radiant damage); Increased Toughness (You can add your Wisdom modifier to your Constitution saving throws).' }],
          10: [{ name: 'Moonlight Step', description: 'As a Bonus Action, you teleport up to 30 feet to an unoccupied space you can see, and you have Advantage on the next attack roll you make before the end of this turn. You can use this a number of times equal to your Wisdom modifier, regaining all uses when you finish a Long Rest.' }],
          14: [{ name: 'Lunar Form', description: 'Improved Lunar Radiance (Once per turn, you can deal an extra 2d10 Radiant damage to a target you hit with a Wild Shape form\'s attack); Shared Moonlight (Whenever you use Moonlight Step, you can also teleport one willing creature within 10 feet).' }]
        }
      },
      {
        ...
        features: {
          ...
          10: [{ name: 'Stormborn', description: 'Your Wrath of the Sea confers two more benefits while active: Flight (Fly Speed equal to your Speed) and Resistance (Resistance to Cold, Lightning, and Thunder damage).' }],
          14: [{ name: 'Oceanic Gift', description: 'Instead of manifesting the Emanation around yourself, you can manifest it around one willing creature within 60 feet. You can manifest the Emanation around both the other creature and yourself if you expend two uses of your Wild Shape.' }]
        }
      },
      {
        ...
        features: {
          ...
          6: [{ name: 'Cosmic Omen', description: 'Whenever you finish a Long Rest, you can consult your Star Map for omens and roll a die. Until you finish your next Long Rest, you gain access to: Weal (Even) - add 1d6 to D20 Tests within 30 feet, or Woe (Odd) - subtract 1d6 from D20 Tests within 30 feet.' }],
  ...
  Fighter: {
    ...
    hitDie: 10,
    savingThrows: ['STR', 'CON'],
    statPriorities: ['STR', 'DEX', 'CON'],
    skillData: { count: 2, options: ['Acrobatics', 'Animal Handling', 'Athletics', 'History', 'Insight', 'Intimidation', 'Persuasion', 'Perception', 'Survival'] },
    masteriesCount: 3,
    ...
    subclasses: [
      {
        ...
        features: {
          ...
          7: [{ name: 'Team Tactics', description: 'When you use Group Recovery, each chosen ally has Advantage on D20 Tests until the start of your next turn.' }],
          10: [{ name: 'Rallying Surge', description: 'When you use Action Surge, choose allies within 30-foot Emanation up to your Charisma modifier. Each can take a Reaction to Attack or Move up to half their Speed.' }],
          15: [{ name: 'Shared Resilience', description: 'When an ally within 60 feet fails a saving throw, you can expend Indomitable to let them reroll with a bonus equal to your Fighter level.' }],
          18: [{ name: 'Inspiring Commander', description: 'Bolstered Rally increases Group Recovery and Rallying Surge area to 60 feet. You have Immunity to Charmed and Frightened.' }]
        }
      },
      {
        name: 'Battle Master',
        ...
        features: {
          ...
          7: [{ name: 'Know Your Enemy', description: 'As a Bonus Action, discern a creature\'s Immunities, Resistances, or Vulnerabilities. Restore by expending one Superiority Die.' }],
          10: [{ name: 'Improved Combat Superiority', description: 'Your Superiority Die becomes a d10.' }],
          15: [{ name: 'Relentless', description: 'Once per turn, when you use a maneuver, roll 1d8 instead of expending a Superiority Die.' }],
          18: [{ name: 'Ultimate Combat Superiority', description: 'Your Superiority Die becomes a d12.' }]
        }
      },
      {
        name: 'Champion',
        ...
        features: {
          ...
          7: [{ name: 'Additional Fighting Style', description: 'Gain another Fighting Style feat.' }],
          10: [{ name: 'Heroic Warrior', description: 'During combat, give yourself Heroic Inspiration when you start your turn without it.' }],
          15: [{ name: 'Superior Critical', description: 'Critical Hits on rolls of 18-20.' }],
          18: [{ name: 'Survivor', description: 'Defy Death: Advantage on Death Saves, rolling 18-20 counts as 20. Heroic Rally: Regain 5 plus Constitution modifier HP at start of each turn when Bloodied.' }]
        }
      },
      {
        ...
        features: {
          ...
          7: [{ name: 'War Magic', description: 'When taking Attack action, replace one attack with casting a Wizard cantrip.' }],
          10: [{ name: 'Eldritch Strike', description: 'When you hit with weapon attack, target has Disadvantage on next saving throw against your spell.' }],
          15: [{ name: 'Arcane Charge', description: 'When you use Action Surge, teleport up to 30 feet.' }],
          18: [{ name: 'Improved War Magic', description: 'When taking Attack action, replace two attacks with casting a level 1 or 2 Wizard spell.' }]
        }
      },
      {
        name: 'Psi Warrior',
        ...
        features: {
          3: [{ name: 'Psionic Power', description: 'Psionic Energy Dice (d6 at level 3, scaling to d12 at level 17). Regain one on Short Rest, all on Long Rest. Powers: Protective Field (reduce damage), Psionic Strike (Force damage), Telekinetic Movement (move objects/creatures).' }],
          7: [{ name: 'Telekinetic Adept', description: 'Psi-Powered Leap: Gain Fly Speed equal to twice your Speed as Bonus Action. Telekinetic Thrust: When you hit with Psionic Strike, force Strength save to be pushed/prone.' }],
          10: [{ name: 'Guarded Mind', description: 'Resistance to Psychic damage. End Charmed/Frightened by expending a Psionic Energy Die.' }],
          15: [{ name: 'Bulwark of Force', description: 'As Bonus Action, grant creatures within 30 feet Half Cover for 1 minute (number equal to Int modifier).' }],
          18: [{ name: 'Telekinetic Master', description: 'Always have Telekinesis prepared, cast without slots/components. Make weapon attack as Bonus Action while concentrating.' }]
        }
      }
    ]
  },
  Monk: {
    ...
    skillData: { count: 2, options: ['Acrobatics', 'Athletics', 'History', 'Insight', 'Religion', 'Stealth'] },
    masteriesCount: 0,
    ...
    subclasses: [
      {
        ...
        features: {
          ...
          6: [{ name: "Physician's Touch", description: 'Hand of Harm can give target Poisoned until end of next turn. Hand of Healing can end one condition: Blinded, Deafened, Paralyzed, Poisoned, or Stunned.' }],
          11: [{ name: 'Flurry of Healing and Harm', description: 'Flurry of Blows can replace each Unarmed Strike with Hand of Healing at no Focus cost. When you deal damage with Flurry, use Hand of Harm without Focus (once per turn). Uses equal to Wisdom modifier per Long Rest.' }],
          17: [{ name: 'Hand of Ultimate Mercy', description: 'As Magic action, touch corpse dead within 24 hours, expend 5 Focus Points. Creature returns with 4d10 plus Wisdom modifier HP. Removes conditions: Blinded, Deafened, Paralyzed, Poisoned, Stunned.' }]
        }
      },
      {
        ...
        features: {
          3: [{ name: 'Shadow Arts', description: 'Expend 1 Focus Point to cast Darkness (see within it, move its area). Gain Darkvision 60 feet (or +60 if already have). Know Minor Illusion with Wisdom as spellcasting ability.' }],
          6: [{ name: 'Shadow Step', description: 'While in Dim Light/Darkness, Bonus Action teleport 60 feet to space in Dim Light/Darkness. Have Advantage on next melee attack.' }],
          11: [{ name: 'Improved Shadow Step', description: 'Expend 1 Focus Point to remove Dim Light/Darkness requirement. Make Unarmed Strike after teleporting.' }],
          17: [{ name: 'Cloak of Shadows', description: 'As Magic action while in Dim Light/Darkness, expend 3 Focus Points for 1 minute. Gain Invisibility, can move through spaces as Difficult Terrain, and Flurry of Blows costs no Focus.' }]
        }
      },
      {
        ...
        features: {
          ...
          6: [{ name: 'Elemental Burst', description: 'As Magic action, expend 2 Focus Points. 20-foot-radius Sphere, 120 feet away. Choose damage type. Creatures make Dexterity save: take three rolls of Martial Arts die on fail, half on success.' }],
          11: [{ name: 'Stride of the Elements', description: 'While Elemental Attunement active, gain Fly and Swim Speed equal to your Speed.' }],
          17: [{ name: 'Elemental Epitome', description: 'While Elemental Attunement active: choose damage Resistance (change each turn); Destructive Stride (Speed +20 feet, deal Martial Arts die damage to creatures you enter space near, damage type chosen); Empowered Strikes (extra Martial Arts die damage once per turn).' }]
        }
      },
      {
        ...
        features: {
          3: [{ name: 'Open Hand Technique', description: 'When you hit with Flurry of Blows attack, impose one: Addle (can\'t make Opportunity Attacks), Push (Strength save or pushed 15 feet), Topple (Dexterity save or Prone).' }],
          6: [{ name: 'Wholeness of Body', description: 'As Bonus Action, roll Martial Arts die and heal for result plus Wisdom modifier. Uses equal to Wisdom modifier per Long Rest.' }],
          11: [{ name: 'Fleet Step', description: 'When you take Bonus Action other than Step of the Wind, also use Step of the Wind immediately after.' }],
          17: [{ name: 'Quivering Palm', description: 'When you hit with Unarmed Strike, expend 4 Focus Points to start vibrations for number of days equal to Monk level. Take action to end them (target Constitution save: 10d12 Force damage on fail, half on success). Only one creature at a time. Can end harmlessly.' }]
        }
      }
    ]
  },
  Paladin: {
    ...
    hitDie: 10,
    savingThrows: ['WIS', 'CHA'],
    statPriorities: ['STR', 'CHA', 'CON'],
    skillData: { count: 2, options: ['Athletics', 'Insight', 'Intimidation', 'Medicine', 'Persuasion', 'Religion'] },
    masteriesCount: 2,
    ...
    subclasses: [
      {
        ...
        features: {
          ...
          7: [{ name: 'Aura of Devotion', description: 'You and allies have Immunity to Charmed in your Aura of Protection.' }],
          15: [{ name: 'Smite of Protection', description: 'When you cast Divine Smite, allies in Aura of Protection have Half Cover until start of your next turn.' }],
          20: [{ name: 'Holy Nimbus', description: 'Bonus Action imbue Aura with holy power for 10 minutes: Advantage on saves vs. Fiends/Undead; Radiant damage to enemies starting turn in aura equal to Charisma plus Proficiency Bonus; Aura becomes sunlight. Long rest to reuse, or expend level 5 spell slot.' }]
        }
      },
      {
        name: 'Oath of Glory',
        ...
        features: {
          ...
          7: [{ name: 'Aura of Alacrity', description: 'Speed increases 10 feet. When ally enters Aura or starts turn there, their Speed increases 10 feet until end of their next turn.' }],
          15: [{ name: 'Glorious Defense', description: 'When you or ally within 10 feet is hit, grant bonus to AC equal to Charisma modifier (minimum +1). If attack misses, make attack against attacker. Uses equal to Charisma modifier per Long Rest.' }],
          20: [{ name: 'Living Legend', description: 'Bonus Action for 10 minutes: Advantage on Charisma checks; reroll failed saving throws; once per turn when weapon attack misses, it hits instead. Long rest to reuse, or expend level 5 spell slot.' }]
        }
      },
      {
        ...
        features: {
          ...
          15: [{ name: 'Undying Sentinel', description: 'When reduced to 0 HP, drop to 1 HP instead and heal for three times Paladin level. Can\'t be magically aged. Once per Long Rest.' }],
          20: [{ name: 'Elder Champion', description: 'Bonus Action imbue Aura with primal power for 1 minute: Enemies have Disadvantage on saves vs. your spells and Channel Divinity; regenerate 10 HP at start of each turn; cast spells with casting time of action as Bonus Action. Long rest to reuse, or expend level 5 spell slot.' }]
        }
      },
      {
        ...
        features: {
          ...
          7: [{ name: 'Aura of Elemental Shielding', description: 'Choose damage type (Acid, Cold, Fire, Lightning, Thunder). You and allies have Resistance in your Aura. Change type at start of each turn.' }],
          15: [{ name: 'Elemental Rebuke', description: 'When hit, Reaction halve damage and force attacker Dexterity save. On fail, they take 2d10 plus Charisma modifier damage of your chosen type. Uses equal to Charisma modifier per Long Rest.' }],
          20: [{ name: 'Noble Scion', description: 'Bonus Action for 10 minutes: Fly Speed 60 feet and hover; when you/ally fails D20 Test, Reaction make them succeed instead. Long rest to reuse, or expend level 5 spell slot.' }]
        }
      },
      {
        ...
        features: {
          ...
          7: [{ name: 'Relentless Avenger', description: 'When you hit with Opportunity Attack, reduce creature\'s Speed to 0 until end of turn. Move up to half your Speed (doesn\'t provoke Opportunity Attacks).' }],
          15: [{ name: 'Soul of Vengeance', description: 'After creature under Vow of Enmity attacks (hits or misses), Reaction make melee attack if within range.' }],
          20: [{ name: 'Avenging Angel', description: 'Bonus Action for 10 minutes: spectral wings, Fly Speed 60 feet and hover; Frightful Aura (Wisdom save or Frightened, attacks against Frightened have Advantage). Long rest to reuse, or expend level 5 spell slot.' }]
        }
      }
    ]
  },
  Ranger: {
    ...
    savingThrows: ['STR', 'DEX'],
    statPriorities: ['DEX', 'WIS', 'CON'],
    skillData: { count: 3, options: ['Animal Handling', 'Athletics', 'Insight', 'Investigation', 'Nature', 'Perception', 'Stealth', 'Survival'] },
    masteriesCount: 2,
    suggestedArray: { DEX: 15, WIS: 14, CON: 13, STR: 12, INT: 10, CHA: 8 },
    progression: {
    ...
    subclasses: [
      {
        ...
        features: {
          3: [{ name: 'Primal Companion', description: 'Magically summon primal beast (Beast of Land, Sky, or Sea). Beast acts on your turn, can move/use Reaction but only Dodge unless commanded. Command with Bonus Action or sacrifice attack to command Beast\'s Strike. If you have Incapacitated, beast acts on its own. Restore dead beast with Magic action and spell slot within 1 hour. Summon new beast on Long Rest.' }],
          7: [{ name: 'Exceptional Training', description: 'When commanding Primal Companion with Bonus Action, it can take Dash, Disengage, Dodge, or Help as its Bonus Action. When beast hits and deals damage, it can deal Force damage instead.' }],
          11: [{ name: 'Bestial Fury', description: 'When commanding Beast\'s Strike, beast uses it twice. First time each turn it hits creature under Hunter\'s Mark, deal extra Force damage equal to spell\'s bonus damage.' }],
          15: [{ name: 'Share Spells', description: 'When you cast spell targeting yourself, also affect Primal Companion if within 30 feet.' }]
        }
      },
      {
        name: 'Fey Wanderer',
        ...
        features: {
          ...
          7: [{ name: 'Beguiling Twist', description: 'Advantage on saves vs. Charmed/Frightened. When you or creature within 120 feet succeeds on such save, Reaction force another visible creature within 120 feet to Wisdom save or be Charmed/Frightened (your choice) for 1 minute.' }],
          11: [{ name: 'Fey Reinforcements', description: 'Cast Summon Fey without Material component. Once without spell slot per Long Rest. Can modify to not require Concentration (1 minute duration).' }],
          15: [{ name: 'Misty Wanderer', description: 'Cast Misty Step without spell slot. Uses equal to Wisdom modifier per Long Rest. When casting Misty Step, bring one willing creature within 5 feet to space near your destination.' }]
        }
      },
      {
        name: 'Gloom Stalker',
        ...
        features: {
          ...
          7: [{ name: 'Iron Mind', description: 'Proficiency in Wisdom saves, or Intelligence/Charisma if already have Wisdom proficiency.' }],
          11: [{ name: "Stalker's Flurry", description: "Dreadful Strike damage becomes 2d8. When using Dreadful Strike: Sudden Strike (attack another creature within 5 feet of original target and within weapon range) or Mass Fear (target and creatures within 10 feet Wisdom save or Frightened until start of your next turn)." }],
          15: [{ name: 'Shadowy Dodge', description: 'When creature makes attack roll against you, Reaction impose Disadvantage. Whether hit or miss, teleport up to 30 feet to unoccupied visible space.' }]
        }
      },
      {
        name: 'Hunter',
        ...
        features: {
          ...
          7: [{ name: 'Defensive Tactics', description: 'Choose one (replace on Short/Long Rest): Escape the Horde (Opportunity Attacks have Disadvantage against you); or Multiattack Defense (when creature hits you, it has Disadvantage on other attacks against you this turn).' }],
          11: [{ name: 'Superior Hunter\'s Prey', description: 'Once per turn when dealing damage to creature marked by Hunter\'s Mark, deal that spell\'s extra damage to different creature within 30 feet of first creature.' }],
          15: [{ name: 'Superior Hunter\'s Defense', description: 'When you take damage, Reaction give yourself Resistance to that damage and same type until end of current turn.' }]
        }
      },
      {
        name: 'Winter Walker',
        ...
        features: {
          ...
          7: [{ name: 'Fortifying Soul', description: 'As Magic action, choose number of creatures equal to Wisdom modifier. Each regains 1d10 plus Ranger level HP and has Advantage on saves vs. Frightened for 1 hour. Once per Long Rest.' }],
          11: [{ name: 'Chilling Retribution', description: 'When hit with attack, Reaction Wisdom save or Stunned until end of your next turn (Speed 0). Uses equal to Wisdom modifier per Long Rest.' }],
  ...
  Rogue: {
    ...
    skillData: { count: 4, options: ['Acrobatics', 'Athletics', 'Deception', 'Insight', 'Intimidation', 'Investigation', 'Perception', 'Persuasion', 'Sleight of Hand', 'Stealth'] },
    masteriesCount: 2,
    ...
    subclasses: [
      {
        ...
        features: {
          ...
          9: [{ name: 'Magical Ambush', description: 'When you cast spell at creature while Invisible, it has Disadvantage on saving throw.' }],
          13: [{ name: 'Versatile Trickster', description: 'When using Cunning Strike Trip option, also use it on another creature within 5 feet of spectral hand.' }],
          17: [{ name: 'Spell Thief', description: 'After creature casts spell targeting you or including you in area, Reaction Intelligence save (DC equals spell save DC). On fail, negate spell\'s effect and steal knowledge if level 1+ and you can cast it. You have spell prepared for 8 hours. Creature can\'t cast it for 8 hours. Once per Long Rest.' }]
        }
      },
      {
        name: 'Assassin',
        ...
        features: {
          ...
          9: [{ name: 'Infiltration Expertise', description: 'Masterful Mimicry: mimic speech/handwriting after 1 hour study. Roving Aim: Speed not reduced by Steady Aim.' }],
          13: [{ name: 'Envenom Weapons', description: 'When using Cunning Strike Poison, target takes extra 2d6 Poison damage on failed save. Damage ignores Poison resistance.' }],
          17: [{ name: 'Death Strike', description: 'When Sneak Attack hits first round, target Constitution save (DC 8 plus Dex mod plus Proficiency Bonus) or attack damage doubled.' }]
        }
      },
      {
        ...
        features: {
          ...
          9: [{ name: 'Phantom Verification', description: 'When you deal Sneak Attack damage, target takes extra 1d6 Necrotic. If target is Incapacitated, extra 3d6.' }],
          13: [{ name: 'Twilight Shroud', description: 'As Bonus Action, become Invisible until end of next turn. If Incapacitated, automatically become Invisible until no longer Incapacitated. Uses equal to Proficiency Bonus per Long Rest.' }],
          17: [{ name: 'Aspect of the Three', description: 'Once per Long Rest, when you would roll Initiative, roll twice and take either result. When you attack Surprised creature, it is Incapacitated until end of its first turn.' }]
        }
      },
      {
        name: 'Soulknife',
        ...
        features: {
          ...
          9: [{ name: 'Soul Blades', description: 'Homing Strikes: when Psychic Blade attack misses, roll Psionic Energy Die and add to roll; if hits, die expended. Psychic Teleportation: Bonus Action, expend die, throw blade up to 10 times number rolled feet to unoccupied space, teleport to that space.' }],
          13: [{ name: 'Psychic Veil', description: 'As Magic action, gain Invisible for 1 hour or until you deal damage/force save. Once per Long Rest, or expend Psionic Energy Die.' }],
          17: [{ name: 'Rend Mind', description: 'When Psychic Blades deal Sneak Attack damage, target Wisdom save (DC 8 plus Dex plus Proficiency) or Stunned for 1 minute (save at end of each turn). Once per Long Rest, or expend three Psionic Energy Dice.' }]
        }
      },
      {
        name: 'Thief',
        ...
        features: {
          ...
          9: [{ name: 'Supreme Sneak', description: 'Cunning Strike option: Stealth Attack (cost 1d6). If you have Invisible from Hide, this attack doesn\'t end Invisible if you end turn behind Three-Quarters Cover or Total Cover.' }],
          13: [{ name: 'Use Magic Device', description: 'Attunement: attune to four magic items. Charges: when using magic item that expends charges, roll 1d6; on 6, don\'t expend charges. Scrolls: use any Spell Scroll, use Intelligence for spells; cast cantrips/level 1 reliably; for higher level, Arcana check DC 10 plus spell level.' }],
          17: [{ name: 'Thief\'s Reflexes', description: 'Take two turns during first round of combat. First at normal Initiative, second at Initiative minus 10.' }]
        }
      }
    ]
  },
  Sorcerer: {
    ...
    statPriorities: ['CHA', 'CON', 'DEX'],
    skillData: { count: 2, options: ['Arcana', 'Deception', 'Insight', 'Intimidation', 'Persuasion', 'Religion'] },
    masteriesCount: 0,
    ...
    subclasses: [
      {
        ...
        features: {
          ...
          18: [{ name: 'Warping Implosion', description: 'Magic action teleport to visible unoccupied space within 120 feet. Creatures within 30 feet of space you left Strength save: 3d10 Force damage and pulled toward space, or half on success. Once per Long Rest, or spend 5 Sorcery Points.' }]
        }
      },
      {
        ...
        features: {
          ...
          6: [{ name: 'Bastion of Law', description: 'Magic action expend 1-5 Sorcery Points create ward with that many d8s. When warded creature takes damage, expend dice reduce damage by total rolled. Ward lasts until Long Rest or use again.' }],
          14: [{ name: 'Trance of Order', description: 'Bonus Action for 1 minute: attack rolls against you can\'t benefit from Advantage; treat d20 rolls of 9 or lower as 10 on D20 Tests. Once per Long Rest, or spend 5 Sorcery Points.' }],
          18: [{ name: 'Clockwork Cavalcade', description: 'Magic action summon spirits in 30-foot Cube (modron-like, intangible, invulnerable): Heal (restore up to 100 HP divided among creatures); Repair (damaged objects repaired); Dispel (end spells level 6 and lower). Once per Long Rest, or spend 7 Sorcery Points.' }]
        }
      },
      {
        ...
        features: {
          ...
          6: [{ name: 'Elemental Affinity', description: 'Choose damage type (Acid, Cold, Fire, Lightning, Poison). Resistance to that type. When casting spell dealing that type, add Charisma modifier to one damage roll.' }],
          14: [{ name: 'Dragon Wings', description: 'Bonus Action for 1 hour: draconic wings, Fly Speed 60 feet. Once per Long Rest, or spend 3 Sorcery Points.' }],
          18: [{ name: 'Dragon Companion', description: 'Cast Summon Dragon without Material component. Once without spell slot per Long Rest. Can modify to not require Concentration (1 minute duration).' }]
        }
      },
      {
        ...
        features: {
          ...
          6: [{ name: 'Absorb Spells', description: 'Always have Counterspell prepared. When target fails save against Counterspell, regain 1d4 Sorcery Points.' }],
          14: [{ name: 'Honed Spellfire', description: 'Spellfire Burst improves: Bolstering Flames adds Sorcerer level to Temporary Hit Points; Radiant Fire damage increases to 1d8.' }],
      ...
      },
      {
        ...
        features: {
          ...
          6: [{ name: 'Bend Luck', description: 'After creature you see rolls d20 for D20 Test, Reaction spend 1 Sorcery Point to roll 1d4 and apply as bonus or penalty.' }],
          14: [{ name: 'Controlled Chaos', description: 'When rolling on Wild Magic Surge table, roll twice and use either.' }],
          18: [{ name: 'Tamed Surge', description: 'After casting Sorcerer spell with spell slot, create effect from Wild Magic Surge table instead of rolling (except final row). Once per Long Rest.' }]
        }
      }
    ]
  },
  Warlock: {
    ...
    savingThrows: ['WIS', 'CHA'],
    statPriorities: ['CHA', 'CON', 'DEX'],
    skillData: { count: 2, options: ['Arcana', 'Deception', 'History', 'Intimidation', 'Investigation', 'Nature', 'Religion'] },
    masteriesCount: 0,
    suggestedArray: { CHA: 15, CON: 14, DEX: 13, WIS: 12, INT: 10, STR: 8 },
    progression: {
    ...
    subclasses: [
      {
        ...
        features: {
          ...
          6: [{ name: 'Misty Escape', description: 'Cast Misty Step as Reaction when taking damage. Additional Steps of the Fey options: Disappearing Step (Invisible until start of next turn or after attack/damage/spell); Dreadful Step (creatures within 5 feet of space left or appeared take 2d10 Psychic).' }],
          10: [{ name: 'Beguiling Defenses', description: 'Immunity to Charmed. After creature hits you, Reaction reduce damage by half and force Wisdom save; on fail, attacker takes Psychic damage equal to damage you took. Once per Long Rest, or expend spell slot.' }],
          14: [{ name: 'Bewitching Magic', description: 'After casting Enchantment/Illusion spell with action and spell slot, cast Misty Step as part of same action without expending slot.' }]
        }
      },
      {
        ...
        features: {
          ...
          6: [{ name: 'Radiant Soul', description: 'Resistance to Radiant damage. When spell deals Radiant/Fire damage, add Charisma modifier to one damage roll.' }],
          10: [{ name: 'Celestial Resilience', description: 'When using Magical Cunning or finishing Short/Long Rest, gain Temporary Hit Points equal to Warlock level plus Charisma modifier. Choose up to five creatures to gain half that amount.' }],
          14: [{ name: 'Searing Vengeance', description: 'When ally within 60 feet makes Death Save, unleash radiant energy. Creature regains half HP maximum and can end Prone. Each creature within 30 feet takes 2d8 plus Charisma Radiant damage and Blinded until end of current turn. Once per Long Rest.' }]
        }
      },
      {
        name: 'Fiend Patron',
        ...
        features: {
          ...
          6: [{ name: "Dark One's Own Luck", description: 'When making ability check or saving throw, add 1d10 after seeing roll. Uses equal to Charisma modifier per Long Rest, no more than once per roll.' }],
          10: [{ name: 'Fiendish Resilience', description: 'Choose damage type (not Force) on Short/Long Rest. Resistance to that type until choosing another.' }],
          14: [{ name: 'Hurl Through Hell', description: 'When hitting creature with attack, transport target through Lower Planes. Charisma save or take 8d10 Psychic damage, Incapacitated until end of your next turn, then return. Once per Long Rest, or expend spell slot.' }]
        }
      },
      {
        ...
        features: {
          ...
          6: [{ name: 'Clairvoyant Combatant', description: 'When forming telepathic bond, target Wisdom save. On fail, Disadvantage on attacks against you, Advantage on attacks against target.' }],
          10: [
          ...
          14: [{ name: 'Create Thrall', description: 'When casting Summon Aberration, modify to not require Concentration (1 minute duration). Aberration has Temporary Hit Points equal to Warlock level plus Charisma modifier. First time each turn Aberration hits creature under Hex, deal extra Psychic damage equal to spell\'s bonus damage.' }]
        }
      }
    ]
  },
  Wizard: {
    ...
    savingThrows: ['INT', 'WIS'],
    statPriorities: ['INT', 'DEX', 'CON'],
    skillData: { count: 2, options: ['Arcana', 'History', 'Insight', 'Investigation', 'Medicine', 'Nature', 'Religion'] },
    masteriesCount: 0,
    suggestedArray: { INT: 15, DEX: 14, CON: 13, WIS: 12, CHA: 10, STR: 8 },
    progression: {
    ...
    subclasses: [
      {
        ...
        features: {
          ...
          6: [{ name: 'Projected Ward', description: 'When creature within 30 feet takes damage, Reaction cause Arcane Ward to absorb that damage. If ward reduced to 0, creature takes remaining damage.' }],
          10: [{ name: 'Spell Breaker', description: 'Always have Counterspell and Dispel Magic prepared. Cast Dispel Magic as Bonus Action, add Proficiency Bonus to check. When casting these spells with slot and they fail, slot not expended.' }],
      ...
      },
      {
        ...
        features: {
          ...
          6: [{ name: 'Extra Attack', description: 'Attack twice when taking Attack action. Cast Wizard cantrip in place of one attack.' }],
          10: [{ name: 'Song of Defense', description: 'When taking damage while Bladesong active, Reaction expend spell slot to reduce damage by five times spell slot level.' }],
          14: [{ name: 'Song of Victory', description: 'After casting spell with action, make one weapon attack as Bonus Action.' }]
        }
      },
      {
        name: 'Diviner',
        ...
        features: {
          ...
          6: [{ name: 'Expert Divination', description: 'When casting Divination spell with level 2+ slot, regain one expended slot of level lower than spell (max level 5).' }],
          10: [{ name: 'The Third Eye', description: 'Bonus Action choose benefit until rest: Darkvision 120 feet; read any language; or cast See Invisibility without slot. Once per Short/Long Rest.' }],
          14: [{ name: 'Greater Portent', description: 'Roll three d20s for Portent instead of two.' }]
        }
      },
      {
        name: 'Evoker',
        ...
        features: {
          ...
          6: [{ name: 'Sculpt Spells', description: 'When casting Evocation affecting visible creatures, choose number equal to 1 plus spell level. Chosen creatures auto-succeed saves and take no damage on successful save.' }],
          10: [{ name: 'Empowered Evocation', description: 'When casting Evocation spell, add Int modifier to one damage roll.' }],
          14: [{ name: 'Overchannel', description: 'When casting damage spell with slot levels 1-5, deal maximum damage on turn cast. First use no adverse effect. Subsequent uses before Long Rest cause 2d12 Necrotic damage per slot level after casting (increases by 1d12 each subsequent use).' }]
        }
      },
      {
        name: 'Illusionist',
        ...
        features: {
          ...
          6: [{ name: 'Phantasmal Creatures', description: 'Always have Summon Beast and Summon Fey prepared. Cast them as Illusion (spectral appearance). Cast without slot (half HP), must finish Long Rest to do so again.' }],
          10: [{ name: 'Illusory Self', description: 'When creature hits you with attack roll, Reaction interpose illusory duplicate. Attack automatically misses, then illusion dissipates. Once per Short/Long Rest, or expend level 2+ slot.' }],
          14: [{ name: 'Illusory Reality', description: 'When casting Illusion spell with slot, Bonus Action make one inanimate nonmagical object in illusion real for 1 minute. Object can\'t deal damage or give conditions.' }]
        }
      }
    ]
  }
...
Object.keys(classData).forEach(className => {
  const cd = classData[className];
  const exportName = className.toLowerCase() + 'En';
  
  out += `\nexport const ${exportName} = {\n`;
  out += `  details: {\n`;
  out += `    name: '${cd.name}',\n`;
  out += `    description: '${cd.description.replace(/'/g, "\\'")}',\n`;
  out += `    traits: [\n`;
  if (cd.hitDie) out += `      { name: 'Hit Point Die', description: 'D${cd.hitDie} per ${cd.name} level' },\n`;
  if (cd.savingThrows) out += `      { name: 'Saving Throws', description: '${cd.savingThrows.join(' and ')}' },\n`;
  out += `    ]\n`;
  out += `  } as DetailData,\n`;
  out += `  hitDie: ${cd.hitDie},\n`;
  out += `  savingThrows: [${cd.savingThrows.map(s => `'${s}'`).join(', ')}] as Ability[],\n`;
  out += `  statPriorities: [${cd.statPriorities.map(s => `'${s}'`).join(', ')}] as Ability[],\n`;
  out += `  skillData: { count: ${cd.skillData.count}, options: [${cd.skillData.options.map(s => `'${s}'`).join(', ')}] as Skill[] },\n`;
  out += `  progression: ${JSON.stringify(cd.progression).replace(/"(\d+)":/g, '$1:')},\n`;
  out += `  subclasses: [\n`;
  cd.subclasses.forEach(sc => {
    out += `    {\n`;
    out += `      name: '${sc.name}',\n`;
    out += `      description: '${sc.description.replace(/'/g, "\\'")}',\n`;
    out += `      features: {\n`;
    Object.keys(sc.features).forEach(lvl => {
      out += `        ${lvl}: [\n`;
      sc.features[lvl].forEach(f => {
        out += `          { name: '${f.name.replace(/'/g, "\\'")}', description: '${f.description.replace(/'/g, "\\'")}' },\n`;
      });
      out += `        ],\n`;
    });
    out += `      }\n`;
  ...
  out += `  ] as SubclassData[],\n`;
  out += `  masteriesCount: ${cd.masteriesCount},\n`;
  out += `  suggestedArray: ${JSON.stringify(cd.suggestedArray)}\n`;
  out += `};\n`;
});
