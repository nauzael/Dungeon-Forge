...
export const XP_BUDGETS: Record<number, { easy: number, medium: number, hard: number, deadly: number }> = {
...
export const getEncounterDifficulty = (totalXP: number, playerLevels: number[]) => {
    let easy = 0, medium = 0, hard = 0, deadly = 0;
    
    playerLevels.forEach(lvl => {
        const budget = XP_BUDGETS[lvl] || XP_BUDGETS[20];
        easy += budget.easy;
